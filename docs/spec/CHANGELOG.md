...
>
> spec / 本文件仅词法、语法、`riu.core`。stdlib 改动不记。
>
> 日常用户教程（`docs/*.md`）的改动不在此记录；语法（`riu/ast/riu.bnf`）改动以 commit log 为准，本文件只记录其在 spec 层的反映。
...
## 2026-09-26 —— 规范范围：词法 / 语法 / `riu.core`

- **修改 [index](index.md) / §10.4**：提案与 spec 仅词法、语法、`riu.core`。stdlib 不进提议、不新写进 spec（既有 io / time 条款保留为历史）。

---

## 2026-09-26 —— 条件编译 + 内置注解形态（#6）

- **修改 §11.1**：`buildAnno` 多参 / 命名，`annoVal ::= expr`（槽内不 parse `type`）。`letAnno` 并入 `buildAnno`；语句前可挂前缀（`#If(true) println(...)`）。`paramAnno` 仍仅零参。新增 [E3119] / [E3163]。
- **新增 §11.16 `#If`**：罩下一条语句或声明；`bool` 常量；假分支 parse 后、Sema 前摘掉。无 `#Else` / `#Elif`。`enum` / 顶层 `type` 别名本轮不接前缀。
- **修改 §11.4.2 / §7.8.1.2 / §12.2.1.2 / §12.4.2.2**：`#Impl` 实参改为无插值字符串或裸 ident（`#Impl("Iter<i32>")` / `#Impl(Eq)`）；缺参 **E2005**（不再误用 E2010）。
- **修改 §11.5.1 / §11.15 / §6.1.3 / §6.6.1.3 / §5.1.1 / §3.8.4.1**：表加 `#If` / `#Anno`；用户 `#Anno struct` 不登记；SDK `anno.ut` 只给人看、不进 `pkg`。
- **附录 A / B / C / D**：A.3 加 `#If` / `#Anno`；B.1 `buildAnno` / B.7 语句前缀；C.6 `#If` / `AnnoType`；D 加 E3119 / E3163，E3112 文案跟上 `#Inline`。
- **冲突 / 兼容**：纯增（`#If`）。既有 `#Impl(Eq)` 仍合法；含 `<` 的必须改成引号（切片 3 已迁仓库）。

## 2026-09-23 —— 结构体字段 const 默认（#23）

- **修改 §7.1.1 / §7.3.1.3 / §7.3.2.2 / §7.3.2.4 / §7.3.3.1–2 / §7.7.2 / §7.10.3.4–5 / §7.11.3**：实例字段可写 const `= expr`；声明处求一次嵌入；字面量省略代入，无默认仍 **E3125**。不引入 `Type()`。**E3129** 只计无默认有名实例字段。`#Val` / `#Frozen` 可带默认。空 `Array<T> = []` 视为零句柄常量。
- **修改 §5.1.4.1.3 / §5.1.5.3 / §8.1.2.5 / §8.10.1 / §9.6.2.1 / §11.11.3.2**：const 位字面量可省略有默认字段；从 v1 不在范围清单去掉字段默认。
- **附录 B / C / D**：`filedDecl` 对齐 bnf 的 `('=' expr)?`；DAA 表述；E3125 / E3129 文案；新增 **E3162**。
- **冲突 / 兼容**：纯增。未写 `= expr` 的既有 struct 仍须列全有名字段。先前语法接受但丢弃的实例字段 `= expr`，现生效。
...
- **修改 §6.2.2 / §6.4 / §6.6.2–3 / §7.1.2.3 / §8.7.4**：extern 允许任意 `Ptr<T>`；隐式堆句柄 / `T&` 只进擦除 `Ptr` / `Ptr<()>`；`Ptr<T>` 可宽化进擦除 `Ptr`。无 `Ptr<T>` → `Ptr<U>`。
- **修改 §9.7.2**：撤回「不引入泛型」。`ptr_of` 返回 `Ptr<payload>`；加 `ptr_cast<T, U>`。句柄可用空结构体作幽灵 `T`。
- **附录 C**：FFI 指针改为 `Ptr<T=()>`。附录 B / D 的 `typeParam` 默认与 E2041 / E2042 已在切片 1–2 写入。
- **冲突 / 兼容**：裸 `Ptr` 继续合法。`let p Ptr = ptr_of(…)` 在 RHS 为 `Ptr<U>` 时 E3014。extern 形参改成 `Ptr<u16>` 后 `String` / `Array` 不再隐式进入。

---

- **修改 附录 B**：使用点少写的实参从左对齐，右侧按声明默认补齐；补不齐仍 **E6011**（类型位 / struct·enum turbofish）或 **E6010**（泛型 fn turbofish）。调用点完全不写 turbofish 仍先推断，失败槽再填默认。
- **冲突 / 兼容**：纯增。未写默认的既有泛型仍须写满实参（`Rc` 照旧 E6011）。

## 2026-09-21 —— 类型形参默认 `= type`（#21 切片 1）

- **修改 附录 B**：`typeParam` 增 `('=' type)?`。有默认须尾部连续（E2041）；默认不得引用更右 / 成环（E2042）。使用点 `genericDefWithRef` 不变。
- **附录 D**：E2041 / E2042。
- **冲突 / 兼容**：纯增。未写默认的既有 `genericDef` 不变；`.ud` 升 v6（旧缓存失效重 parse）。
...
- **附录 A.3 / C / D**：`#Packed` / `#Align`；术语 overlay；E2038 / E2039 / E2040；E3108 收 `#Align`；E6018 泛化为 `{}`。
- **冲突 / 兼容**：纯增。未标 `#Packed` / `#Align` 的默认布局不变。

---

## 2026-09-20 —— `_` 丢弃名（#20）

- **新增 §1.4.3.1 / 修改 §5.1 / §5.5.4 / §6.1.1.1 / §6.2.1.3 / §7.1.1.3 / §7.3.2.2 / §7.3.2.4 / §7.3.3.2 / §7.10.3.4 / §10.3.2.1 / §3.10.7.2 / §13.1**：恰好 `_` 是丢弃槽（字段 / 变量名），可重复、不绑定；禁止类型 / 函数 / 全局等声明名（**E3161**）。`_` 字段字面量可省并零填；`T::fields` 不收录。match 兜底仍 `else`。
- **附录 C / D**：术语「丢弃槽」；E3161；E3129 改为「有名实例字段」。
- **冲突 / 兼容**：原先恰好 `_` 是私有标识符（可 `s._`、要唯一）。现有代码若把 `_` 当名字读/写会变成 E3040 / E3030；重复 `_` 字段 / 形参变为合法。
