...
> 与 [`riu/ast/riuLexer.g4`](../../riu/ast/riuLexer.g4) 的 lexer token 一致；冲突以 `.g4` 为准。本附录是规范化摘录。

## A.1 关键字

下表列出 `riu.g4` 中作为独立 lexer token 声明的关键字（按字母序）：
...
§A.1.2 关键字**不得**用作 `ID`（标识符）。`riu.g4` 的 lexer 优先级保证关键字命中先于 `ID`。

## A.2 上下文标识符

下列名称在 `riu.g4` 中**不是**关键字（即可作 `ID` 出现），但在特定上下文具有语言层语义；用户**应当**避免在普通命名中复用：
...
下表是 `riu.g4` 中的符号 lexer token，按字母序列出。规范在 §1.7 给出语义概览，本表只列形态与 token 名。
...
完整定义见 `riu.g4` 末尾。下表只列规范层常引用的 token：
