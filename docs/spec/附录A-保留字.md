...
> 与 [`riu/ast/riu.bnf`](../../riu/ast/riu.bnf) 的 lexer token 一致；冲突以 `riu.bnf` 为准。本附录是规范化摘录。

## A.1 关键字

下表列出 `riu.bnf` 中作为独立 lexer token 声明的关键字（按字母序）：
...
§A.1.2 关键字**不得**用作 `ID`（标识符）。`riu.bnf` 的 lexer 优先级保证关键字命中先于 `ID`。

## A.2 上下文标识符

下列名称在 `riu.bnf` 中**不是**关键字（即可作 `ID` 出现），但在特定上下文具有语言层语义；用户**应当**避免在普通命名中复用：
...
§A.2.1 上述名称由 `base.ut` 注册。用户**可以**在本文件定义同名顶层符号：裸名走 §10.2.5（L1 盖住默认 `riu.core.*`）；SDK 侧用路径（§10.4.4.2）。
...
下表是 `riu.bnf` 中的符号 lexer token，按字母序列出。规范在 §1.7 给出语义概览，本表只列形态与 token 名。
...
完整定义见 `riu.bnf` 词法段。下表只列规范层常引用的 token：
...
§A.6.3 上述名称未来如引入**应当**升格为关键字并同步更新本附录与 `riu.bnf`。
