...
状态：**已全面落地**，2026-05-09。spec 正文回写见 §3.11 / §4.8.4 / §4.11 / §6.1.1.6 / §6.5.5 / §6.6.2.1 / §8.1.1.1 / §8.1.2.5 / §8.7.6 / §8.9 / §8.10；编译器实现 / `src/yux*.g4` / `tests/cases/lambda_*` 用例 / 用户教程 `docs/Lambda与闭包.md` 全部就位。本草案保留为决议史档。

> **2026-08-26**：lambda 字面量形态收敛为 `(args) Ret? => expr` / `=> { stmts }`，尾随仅挂在调用上（见 spec §4.11 / CHANGELOG）。本草案正文保留落地时的旧写法。

> **2026-08-25**：类型位语法已改为 `Function<P..., Ret>`（见 `docs/dev/function-type-impl-log.md`）。本草案正文仍用落地时的 `fn(T)R` 写法。

> **2026-06-10**：§5（函数值 RC/ABI）与 §6（闭包）已提炼为独立草案 `DRAFT-closure-capture.md`，闭包捕获模型的后续修订以那篇为准。本文 §5–§6 不再独立演进。

> 落地分布：§3 函数类型 / §4 lambda 字面量 / §6 闭包（→ DRAFT-closure-capture.md）/ §8 RC 与借用 —— 详见末尾"决议日志"与 `CURRENT.md` 进度条目（2026-05-08 ~ 2026-05-09）。
...
g4 实际产生式由 Phase 1 落地时同步（按 RULES.md 须用户拍板，本草案仅给概念形）。
...
{ body }                           ; 0 参块；无 =>；必须多行真块（单行 { expr } 是表达式位置而非 lambda）
...
> 改语法属高风险动作，按 RULES.md 须用户确认；本节只列要改什么，不动手。
...
3. `src/yux*.g4` 同步（按 RULES.md 用户拍板后改）；
