...
涉及章节（预估）：§4.11.6、§6.5.5、§8.7.6、附录 D。
...
- **O3**：`Heap<T>?` 捕获在 body 内多次调用的所有权语义——当前 env 独占所有权，首次调用后 outer slot 已 null，逻辑一致；但若 lambda 自身被多次调用且 body 内多次读捕获的 Heap 字段，行为符合预期（env 持有），无需额外条款。
- **O4**：`[T * N]` 定长数组的捕获——当前按值类型（struct 等价）走 move 档，字段级复制；定长数组元素含堆句柄时，字段级 retain 覆盖，行为正确。是否需要单独列一行 → 否，归入 §4.1 move。
