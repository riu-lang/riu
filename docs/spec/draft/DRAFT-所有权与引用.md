...
| **堆句柄** | `Box<T>` / `Weak<T>` / `Array<T>` / `String` | 堆（RC 头 + payload） | 句柄复制 + RC 调整 | 共享语义；要独立缓冲走显式 `.clone()`（`String` 不可变，无 clone） |
...
- 堆句柄类型是封闭集合：`Box`、`Weak`、`Array`、`String`，不允许用户自定义新的"堆句柄"
- 自引用 / 双向引用结构体**必须经堆**（`Box<Self>?` / `Weak<Self>?`），栈上自引用直接被语法拒绝（无办法表达）
...
| `&box`（box 是 `Box<T>` 局部变量） | ✅ | `Box<T>&` | 借用**句柄本身**，不是 payload；详见 §4.3 |
...
附加约束：在 `T&` 的存活区间内，其根对象不可被赋值 / 移交。Box 句柄的"赋值"等价 release-旧 + retain-新，借用期内禁此操作即可（纯本地检查）。
...
- **不可空**：`Box<T>` 永远指向有效对象；可空写 `Box<T>?`
- **共享**：句柄复制 = retain；多个 Box 句柄共享同一 block

### 4.2 访问形态（T&-风）

`Box<T>` **没有自己的方法**——所有 `box.field` / `box.method(args)` 都自动解引用到 payload `T` 上分发：
...
- **`&box` → `Box<T>&`**（借用 Box **句柄本身**，不是 payload）。`Box<T>` 是单字句柄值，`&box` 就是对该值取址，得到 `Box<T>&`。可传给 `fn f(b Box<T>&)` 这类接受句柄借用的函数；callee 通过自动解引用访问 payload
- **`&box.field` → `FieldType&`**（借用 payload 的某个字段）。`box.field` 自动解引用到 payload 后取字段，再 `&` 取址——结果绑到 box 句柄存活期；详见 §3.4 / §3.5
- **没有直接得到"整个 payload `T&`"的语法**。要把 Box 的 payload 整体借给接收 `T&` 的函数，有两条路：
  1. 把函数签名改为接收 `Box<T>&`（推荐，调用方写 `f(&box)`，零开销；callee 用自动解引用访问字段）
  2. 把函数拆成接收具体字段引用，调用方写 `f(&box.field)`
- **FFI 走 Ptr**：`ptr_of:<Box<T>>(box)` 直接给到 payload 首地址（跳过 RC 头），见 §9.4。这是用户层唯一能拿到"裸 payload 指针"的口子，且仅 FFI 边界使用

## 5. `Weak<T>` 弱引用

### 5.1 范围（v1）

- **`Weak<T>` 仅配 `Box<T>`**：`Weak<Array<T>>` / `Weak<String>` v1 不支持
- TODO 后续按需扩展到其他堆句柄类型
...
| 操作 | 形式 | 说明 |
|---|---|---|
| 构造函数 | `Weak(box Box<T>?)` | 传 `null` 得空 weak；传活 box 绑定并 retain weak 计数 |
| 内置函数 | `weak:<T>(box Box<T>?) Weak<T>` | 编译器 baked，与构造函数等价 |
| 升级 | `upgrade(w Weak<T>) Box<T>?` | 强引用还在则返回 Some，已释放或空 weak 返回 None |
...
- **upgrade**：`if (strong > 0) { strong++; return Box(handle); } else null`
...
- **平凡结构体**（递归地不含任何 `Box` / `Weak` / `Array` / `String`，也不含含上述的子结构体）→ 按位 memcpy / 直接析构，快速路径
- **非平凡结构体** → 字段级处理：
...
主要服务于 `String` 字面量；`Array` / 用户 `Box` 暂未走静态字面量路径，TODO 视需要扩展。
...
| 位置 | 允许 | 不允许 |
|---|---|---|
| 参数 | 内置标量、`Ptr` | `T&` / `Box<T>` / `Array<T>` / `String` / `Weak<T>` / 用户 struct |
| 返回值 | 同上 + `void` | 同上 |
...
`T&` / `Box<T>` / `Weak<T>` / `Array<T>` / `String` 作实参传给 `Ptr` 形参时**自动转换**（隐式仅此一处例外，文档明确说明）：
...
| `Box<U>` | 指向 U payload | **跳过 RC 头**；C 端只能读写 payload，不能 free |
| `Array<U>` | 指向元素首地址 | 同上 |
...
- **地址相等**：内置 `same_ref:<T>(a, b) bool`，编译器 baked
  - 接受 `Box<T>` / `Weak<T>` / `T&` / `Array<T>` / `String`
  - 比对底层 handle 指针
  - 空 Weak 与任何活句柄不等
...
- `sdk/yux/src/yux/core/base.yux`：`Array<T>` / `String` / `Box<T>` / `Weak<T>` 语义和方法签名重写
- `Weak(box Box<T>?)` 构造、`upgrade` builtin、`weak` builtin
...
- `docs/类型系统.md`：四档体系；`T&` / `Box<T>` / `Weak<T>` / `Array<T>` / `String` / `Ptr` 各自语义重写
...
- **[#1.A]** 类型档位锁定四档：值（含用户 struct，默认按值复制）/ `Box<T>` / `T&` / `Ptr`（去泛型，等价 `void*`，仅 FFI 单向）
...
- **[#1.I]** 自引用结构体必须经堆 `Box<Self>?` / `Weak<Self>?`
- **[#1.J]** Box 无自有方法，`box.field` / `box.method()` 一律解引用到 T 上分发
- **[#2.1]** RC 头 `{ strong: u32, weak: u32 }` 8 字节
- **[#2.2.a']** Weak 原生可空，`Weak<T>?` 禁；构造函数 `Weak(box Box<T>?)`，builtin `weak:<T>(box)`；空 weak 写 `Weak(null)` / `weak(null)`
- **[#2.2.b]** Weak `==` / `!=` v1 禁用
...
- **[#2.3.E.2]** Weak v1 仅配 Box；Array / String 的 weak 不暴露但 layout 保留 weak 字段；TODO
- **[#2.3.E.3]** 字面量哨兵 `0xFFFFFFFF`，retain / release 入口对比跳过
- **[#3.A]** `Array<T>` / `String` 归堆句柄类，与 `Box` / `Weak` 同规则
...
- **[#4.3 修订]** `&box` 是 `Box<T>&`（句柄借用），不是 `T&`；没有"借用整个 payload 为 T&"的直写语法；要传 payload 给 T& 形参的函数，要么改签名为 `Box<T>&`，要么走 `&box.field` 拆字段；裸 payload 指针仅 FFI 通过 `ptr_of:<Box<T>>(box)` 获取
