...
状态：**已落地（Phase 1–6，2026-06-05）**，见 §5.1.4（全局 `let` 三档）、§7.11（struct 静态字段）、§11.9（`#Mut` 全局位/静态字段位）、§11.11（`#Static` 字段位）。草案保留为历史档（决议日志追溯）。
日期：2026-05-26。
...
- **静态字段无 `let` 关键字**：沿用结构体字段段"无 let"约定（[yux-syntax.md](../../../rules/yux-syntax.md)）
