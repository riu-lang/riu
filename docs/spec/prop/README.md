...
范围：**词法**、**语法**（含语言语义 / `riu.bnf`）、**`riu.core`**。stdlib / net / 工具链 **不进提议、不进 spec**（以源码为准）。表中历史条目不按本范围扩、不回写 spec。
...
未到 `待实施` 不要改 spec 正文 / `riu.bnf`。改 `riu.bnf` 仍须先停下来拍板。
...
| 2 | `Iter<T, E>` / 泛型 for-in | 已落地 | [2-iter.md](2-iter.md) |
