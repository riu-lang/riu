...
待实施后写入 §10.1.1。草案即上文：顶层身份、单 `[library]`、`[[executable]]`、`external_links` 的 `//` / `./`（配置不写库后缀，驱动按目标补）、`dll` 只复制、CLI、输出路径、与 `[fmt]` 的边界。已回写。
...
- 2026-09-22（`notes/0.23`）：回写 §10.1.1 / §10.1.2.1 / 附录 C/D / 用户文档；提案标已落地。
