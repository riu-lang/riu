...
> 注（2026-09-04）：文中 `#Fallible(E)` 为 v0.20 旧形态；v0.21 起失败声明为签名后缀 `T ! E`（见 `fallible-syntax-impl-log.md`）。下文保留历史表述。

本文件归档 yux 错误模型 v1 的实施记录：依赖图、各 Phase 决议（[#1]-[#9] + 复审 [#R]）、Phase 10 落稿子项 10a-10i。是后续回答"`T ! E` 签名形态"、"`!` 与 `try-catch` 的关系"、"main 错误退出 wrapper 在哪生成"、"为什么 panic 不进错误通道"等问题的事实来源。
