...
dest = root / "build" / "dependences" / "utils"
dest.parent.mkdir(parents=True, exist_ok=True)
subprocess.run(["git", "-c", "core.autocrlf=false", "clone", "-q", "--", str(origin), str(dest)], check=True)
subprocess.run(["git", "-c", "core.autocrlf=false", "-C", str(dest), "checkout", "-q", "--detach", rev], check=True)
with (dest / "src" / "utils.ut").open("a", encoding="utf-8", newline="\n") as f:
    f.write("\n; dirty\n")

(root / "riu.toml").write_text(
    f'''name="dep_git_dirty"
version="1.0.0"
