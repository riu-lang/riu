...
All notable changes to the "riu-vscode" extension will be documented in this file.
...
- 版本：riu-lsp → 0.1.1、riu-vscode → 0.9.1、riu-idea → 0.5.1-SNAPSHOT、riu-lang-lsp → 0.1.1
...
- 同步 v0.9 语法 + 修复参数 / 返回类型颜色丢失
  ...
  - 错误传播 `e()!` 单独命名为 `keyword.operator.error-propagate.ut`，便于配色
...
- 编译器附带：formatter 修复
    ...
    - `riu format <file>` CLI 在向上找 `riu.toml` 时遇到根目录会无限循环，已修
...
- 元组（tuple）与类型别名（type alias）相关高亮
  ...
  - 元组成员访问 `a.0` / `a.0.0`：`.N` 整体识别为 `meta.tuple.member.ut`，N 落到 `variable.other.tuple-member.ut`，避免被识别为 float 或纯数字常量
...
- 语言服务全部由编译器 `riu lsp` 子命令提供（stdio JSON-RPC）
- 新增配置项 `riu.executablePath` 指定 riu 可执行文件路径，留空走 PATH
- 当前由服务器提供能力：诊断、文档符号、格式化、静态补全（关键字 / 类型 / 内置函数 / snippet）
- 跳转 / hover / signatureHelp / 动态补全将在后续版本由服务器补齐

## [0.2.0]

- 用基于 `riu/ast/riu.g4` 生成的 antlr-ng parser（通过 `-D language=TypeScript` 覆盖 .g4 的 Cpp 目标）替换原正则方案
...
- 跨文件跳转与补全：按 `riu.toml` 定位项目根，展开 `use a.b` / `use a.b.*`；sdk 从 PATH 上 `riu(.exe)` 所在目录的 `../sdk` 回退
- hover：显示变量 / 函数 / struct / 方法签名
- signatureHelp：函数调用 `f(|)` 显示参数列表并跟随 `,` 高亮当前参数
- 文件监听：`.ut` 保存 / 变更自动失效跨文件解析缓存
