...
```
AST（数据 + 注解槽）  ←  Generic（替换栈 + struct 实例表；fn 表仍在 Compiler）  ←  Sema / Analyzer
                              ↑
                          Codegen（LLVM）
```

名字查找：`riu/ast/name_lookup.h`（已加载模块图，0 LLVM）。类型计算由 Sema 写槽；codegen 读槽，泛型 subst 帧用 `structuralType()` 回退。`riu_generic` 0 LLVM；`applySubst` / 替换栈 / struct 实例表在 generic，fn 实例表 2.4 起迁入。
