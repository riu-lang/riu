...
// yux-test-runner.exe — 独立测试运行器（单 DLL 模式）
//
// 加载单个 *.test.dll，顺序执行其中所有测试并打印结果。
// 由 `yux test` 多子进程并行调用，每个子进程负责一个 DLL。
//
// 用法：yux-test-runner.exe <dll-path> [--verbose]
//
// DLL 协议：
//   LoadLibrary → GetProcAddress("yux_test_init") 调之
//   → GetProcAddress("yux_test_get_count") 取测试数
...
int main(int argc, char* argv[]) { // NOLINT(bugprone-exception-escape)
    ...
    // 调 yux_test_init
    auto initFn = reinterpret_cast<void (*)()>(GetProcAddress(hDll, "yux_test_init"));
    if (!initFn) {
        std::cerr << "ERROR: yux_test_init not found in " << opts.dllPath << "\n";
        FreeLibrary(hDll);
        return 1;
    }
    initFn();
    ...
    // 取访问器
    auto countFn = reinterpret_cast<int (*)()>(GetProcAddress(hDll, "yux_test_get_count"));
    auto nameFn = reinterpret_cast<const char* (*)(int)>(GetProcAddress(hDll, "yux_test_get_name"));
    auto fnFn = reinterpret_cast<void (*(*)(int))()>(GetProcAddress(hDll, "yux_test_get_fn"));
