...
// 运行时辅助函数实现
//
// - yuxrt C 符号声明（yuxrt_alloc / yux_rc_*）
// - Weak / Dyn / typed Rc release / Array / Heap / 启动 stub 的 IR
//
// Rc 的 alloc/retain/generic release 在 yuxrt.lib（ffi/yux_rc.c），不再 emit IR 体。
...
// 计数器定义在 yuxrt；此处只 load/store 该 extern。
...
// yux_rc_alloc(payload_size i64) -> ptr  （yuxrt.lib）
llvm::Function* getRcAllocFn(llvm::Module* module, llvm::IRBuilder<>& builder) {
    string fnName = "yux_rc_alloc";
    auto func = module->getFunction(fnName);
    if (func) return func;
...
// yux_rc_release(ptr block) -> void（无 payload 析构）
llvm::Function* getRcReleaseFn(llvm::Module* module, llvm::IRBuilder<>& builder) {
    string fnName = "yux_rc_release";
    auto func = module->getFunction(fnName);
    if (func) return func;
...
// ==================== Rc 辅助函数实现 ====================
// alloc/retain/generic release 在 yuxrt（yux_rc_*）。typed / dtor / dyn 只在 strong 归零时跑 payload 析构，再 drop。
...
static void emitReleaseViaC(llvm::LLVMContext& context, llvm::IRBuilder<>& builder, llvm::Module* module,
    ...
    auto needFn = getYuxRcReleaseNeedDtorFn(module, builder);
    auto dropFn = getYuxRcDropBlockFn(module, builder);
