...
int runViaJIT(std::unique_ptr<llvm::Module> mod, std::unique_ptr<llvm::LLVMContext> ctx,
    ...
    auto jitOrErr = llvm::orc::LLJITBuilder().setObjectLinkingLayerCreator(&makeYuxObjectLinkingLayer).create();
    if (!jitOrErr) {
        llvm::errs() << "[jit] LLJIT create failed: " << llvm::toString(jitOrErr.takeError()) << "\n";
        return 1;
    }
    auto& jit = *jitOrErr;
    ...
    if (!procGen) {
        llvm::errs() << "[jit] process generator failed: " << llvm::toString(procGen.takeError()) << "\n";
        return 1;
    }
    ...
    if (auto e = jit->addIRModule(std::move(mainTsm))) {
        llvm::errs() << "[jit] addIRModule(main) failed: " << llvm::toString(std::move(e)) << "\n";
        return 1;
    }
    ...
    for (size_t i = 0; i < extraMods.size(); ++i) {
        ...
        if (auto e = jit->addIRModule(std::move(tsm))) {
            llvm::errs() << "[jit] addIRModule(extra) failed: " << llvm::toString(std::move(e)) << "\n";
            return 1;
        }
    }
    ...
    if (!sym) {
        llvm::errs() << "[jit] lookup mainStartup failed: " << llvm::toString(sym.takeError()) << "\n";
        return 1;
    }
