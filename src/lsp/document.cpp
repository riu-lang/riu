...
public:
    DiagnosticListener(const std::string& docText, std::vector<Diagnostic>& out)
        : _text(docText), _out(out) {}
    ...
    void syntaxError(antlr4::Recognizer* /*recognizer*/, antlr4::Token* offending,
        ...
        if (offending) {
            ...
            if (d.end.line < d.start.line ||
                (d.end.line == d.start.line && d.end.character < d.start.character)) {
                d.end = d.start;
            }
        } else {
            d.end = d.start;
...
void contextRange(const std::string& text, antlr4::ParserRuleContext* ctx,
                  LspPosition& start, LspPosition& end) {
    auto* a = ctx->getStart();
    auto* b = ctx->getStop();
...
void collectSymbols(const std::string& text, ::yux::yuxParser::ProgramContext* prog,
