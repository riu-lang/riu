...
- **编译期单态化分发**（v0.4 阶段不上虚表 / `dyn Draft`）
- 与现有泛型实例化路径融合
...
- 借用作用域规则规范化（沿用 `docs/spec/draft/DRAFT-所有权与引用.md` §3.5 的 O(1) 唯一规则，落 spec §8.6）
- 防 dangling 的静态检查（深度按需）
- 与 `Box<T>` / `Array<T>` 互操作的边界
- 借用与泛型边界匹配的接口规则（为 v0.5 draft 提供前置规则）

**退出标准**：

- spec §8.6 / §8.9 完成；新增反例测试集
- `T&` 与泛型边界 / 方法分发的交互在 spec 中有明文规则，v0.5 draft 草案可直接引用
