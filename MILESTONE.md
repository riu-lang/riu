# 里程碑规划

> 内容多、短期难完，版本号从 **v0.x** 起步推进，**v1.0** 定义为"语言核心稳定 + 规范定稿 + 基础 SDK 可用"的状态，不设硬性日期。
> 当前条目仅为方向规划，不是承诺；具体实施进度看 `CURRENT.md`，已完成实现细节归档到对应 `*-实现.md`。

---

## 稳定版目标（参考）

### 语言参考

- `c` `c++` `rust` `kotlin` `java` `python` `go` 不分先后
- 主要参考 `rust` `c++`

### 特性

- 明确的使用
  - 知道每一步类型，在干什么
  - 没有隐式转换
  - 布局而不是继承
  - 移动 or 复制
  - 栈 or 堆
- 统一的用法
  - 尽可能少的特例规则
- 简约的代码
  - `kotlin` 思维
- 内存安全
  - 不直接接触指针
  - 内存由编译器管理，用户只关心业务代码，以及把数据放栈还是堆
  - 指针单向转换 `yux -> ptr`
  - 限制外部函数（后期内容），类 `rust`
- 代码生成代码（后期内容）
  - 类 `rust`
  - 类 `java` 注解预处理
- 更强大的接口 `draft`
  - 接口一致
  - 像接口
- ~~无异常设计（待定）~~
- 文档：md 格式
- 条件编译
- 代码修复建议
- 测试

---

## 版本路线（草案）

每个版本只列**主题** + **范围** + **退出标准**。范围之外的项不在该版本承诺内。

### v0.1 — 核心模型稳定（进行中）

**主题**：所有权 / RC / 内置堆类型 layout 收口。

**范围**：
- Box / Array / Weak / String / Ptr 的 Block + RC 协议（strong/weak + 哨兵）
- callee-clean retain/release、move-return 注入
- 泛型函数与结构体方法实例化
- 字符串不可变 + StringBuilder
- 析构序与作用域规则

**退出标准**：
- `所有权与引用-P1实现.md` 全部 Phase 勾完
- RC 泄漏检测通过（参见已存在的 RC leak 检测能力）
- `xmake test` 全绿

### v0.2 — 文档与规范骨架

**主题**：从教程升格到规范，对外可参考。

**范围**：
- `docs/spec/` 主体章节（§1–§7、§9）填实
- `DRAFT-所有权与引用.md` → `docs/spec/08-所有权与引用.md` 迁移

**退出标准**：spec 各主体章节 TODO 清零（Open Issues 除外）；附录 B 与 `yux.g4` 对齐。

### v0.3 — 编译诊断改进

**主题**：把"编译报错信息太少"这件事系统性补上。诊断质量影响所有后续开发体验，单列一档。

**范围**：
- 错误统一格式：`文件:行:列 [错误码] 主信息`
- 出错处源码片段 + 下划线/插入符指向
- 错误码体系（`E0001` 起，按类别分段：词法 / 语法 / 类型 / 所有权 / 模块 …），写入 `docs/spec/附录D-诊断.md`
- warning vs error 分级雏形（含静默 / 升级开关）
- 高频场景的"提示 / 修复建议"：
  - 类型不匹配（含 Nullable / 泛型实参）
  - 未声明标识符（含拼写近似建议，如可行）
  - 缺失 `;` / 空格规则违反
  - 泛型实参缺失或多余
  - `T&` 出现在禁止位置（字段 / 返回值 / `T& &`）
  - `Weak<T>?`、`Weak == / !=`
  - `extern` 签名不匹配
- 诊断回归测试集：`tests/cases/diag_*.{yux,expected_err}`

**退出标准**：
- 上述高频场景全部带行列 + 提示
- 诊断格式在附录 D 中规范化
- 诊断回归测试纳入 `xmake test`

### v0.4 — 语法/语用可用性

**主题**：让"日常代码"少一些不便。

**范围**：
- 字符串模板（`$ident` / `${expr}`，候选见 `TARGETS.md`）
- `for` 迭代（语法选型 + Array / 定长数组特例）
- `format` 类格式化（候选 `?` / `{}`）
- 改进的 `if` 表达式语义（如尚未规范化）

**退出标准**：以上语法在 spec §4–§5 落规范文字、`docs/*` 教程同步、含示例与测试。

### v0.5 — `draft`（接口/约束）初版

**主题**：参考 Rust trait + Java interface，落最小可用形态。

**范围**：
- `draft` 声明、`Type : Draft` 实现块
- 用作参数类型 / 泛型边界
- 编译期分发（暂不上虚表 / 动态分发）
- 与现有泛型实例化路径融合

**退出标准**：`Counter : ToString` 类示例端到端通过；spec 增 §X 章节。

### v0.6 — SDK 第一轮扩充

**主题**：把"能写实际程序"的最小标准库铺齐。

**范围**：
- `math.*`
- `Map<K, V>`
- `String` 方法补全（`to_upper` / `split` / `contains` 等）
- 基础 IO 改进（行读取、文件 API 雏形）

**退出标准**：上述模块每条方法配测试与 spec §9 条款。

### v0.7 — 借用与生命周期细化

**主题**：把 `T&` 从"语法允许"推到"语义清晰"。

**范围**：
- 借用作用域规则规范化
- 防 dangling 的静态检查（深度按需）
- 与 `Box<T>` / `Array<T>` 互操作的边界

**退出标准**：spec §8.6 / §8.9 完成；新增反例测试集。

### v0.8 — 工具链与编辑器支持

**主题**：提升开发体验。

**范围**：
- LSP / IDE 插件：高亮、补全（表达式调用）、跳转
- 测试框架（约定 + runner）
- 文档生成（从源码注释 / spec 抽取）

**退出标准**：在示例项目中可用；至少一个编辑器（VSCode 或 IDEA）端到端体验通过。

### v0.9 — 条件编译 + 注解可扩展

**范围**：
- 条件编译指令
- 用户自定义注解雏形（仅元数据，尚不允许执行）

### v0.10 — 代码生成代码（实验）

**主题**：注解预处理 / 编译期生成。

**范围**：
- 类 Java 注解处理 / 类 Rust 派生宏的最小子集
- 仅允许产出代码、不允许任意副作用

**退出标准**：派生 `ToString` 等示例可用。

### v0.11 — 性能与 layout 优化

**范围**：
- String 专属 FAM Block（`{strong, weak, len_cps, u32 data[]}`）
- Array Block 内联小尺寸优化（如可行）
- 内联策略与裁剪

**退出标准**：基准测试无回归；新增微基准记录。

### v1.0 候选 — 规范定稿 + ABI 冻结

**退出标准**：
- spec 全部章节 *Open Issues* 清空或转入 v1.x
- 调用 ABI / 内存模型在该版本内冻结
- `base.yux` SDK 公开方法签名稳定
- 所有 BUGS.md 中的 *blocker* 清零

### v1.x 与之后（不在本规划展开）

- 异步 / 并发原语
- 多目标（除 Windows 外）
- 包管理与远程依赖
- ……

---

## 术语

- **退出标准**：进入下一版本前必须满足的条件，非"完成度"而是"门槛"。
- **范围**：本版本承诺触达的领域；范围外的提议默认下放。
