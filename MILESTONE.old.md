...
v0.21 及更早按**主题 + 范围 + 退出标准**归档。v0.22 起落地见 `notes/`，本文件不重复。
...
> **现状提示**：v1.0 仍远（当前 v0.23-alpha）。spec 残留 Open Issues 已审计并关/转/留；本节标准是终态门槛，不是近期目标。下方"后续主题"里的待编号工作要先落，才进 v1.0 候选。
...
- **SDK 向 v1**：pkg 公开面、路径 / 环境 / String·Array·Map·Set / `yux.io` / `yux.time` 已随 **v0.22** 落地一截。仍缺：`format` 类 printf（候选 `?` / `{}`）、正则、`Command` / JSON、`Iter<T>`。后续按两周节奏按需补，不锁死下一版清单。
...
| 版本 | 状态 | 记录 |
|------|------|------|
| v0.23.0-alpha | 进行中，约 2026-09-27 | [notes/0.23.md](notes/0.23.md) |
| v0.22.0 | ✅ 2026-09-13 | [notes/0.22.md](notes/0.22.md) |
...
- [x] 不依赖手写 C 即可编写并运行 ≥3 个独立 CLI 示例（stdin/stdout/文件/退出码）
- [x] `continue` + `for-in`（Array / 定长数组）有 SDK 测试与 spec 条款
- [x] `yux.io`（或等价）read/write 有 SDK 测试；stderr 通道可用
- [x] `Map<K,V>` 基本 get/insert/contains 可用（性能不承诺）
- [x] FFI 示例项目 + extern 边界文档与 `yux test` / `yux-check test` / `./build.ps1 test` 全绿
- [x] `./lint.ps1` 0 warnings；CHANGELOG 收口
