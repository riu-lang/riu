...
- 泛型 fn/impl 体已走 SemaPass：类型参数当不透明 TypeParam。形态检查（#NoCopy / 未定义符号 / arity）在模板体报；ret / 赋值在调用点 typeArgs 已知后 `substitute` 再查（E3014）。依赖 T 具体化的方法分派 / 运算符仍留给实例化期 / Compiler。
...
- 缺口（sema 不报、靠 codegen）：泛型体内依赖 T 具体化的方法分派 / 运算符。调用点签名替换与 ret / 赋值已由 SemaPass 做。这些路径的 throw 写 Compiler。
