...
- 泛型 fn/impl 体已走 SemaPass：类型参数当不透明 TypeParam。形态检查（#NoCopy / 未定义符号 / arity）在模板体报；ret / 赋值 / 方法分派 / 内置运算符在调用点或写出的 `S<Concrete>` 类型位置 `substitute` 后再查（E3014 / E3095 / E3001）。
...
- 缺口（sema 不报、靠 codegen）：源码从未写出 `S<Concrete>` 且无调用时，泛型 struct 方法体内依赖 T 的类型错两边都不查（与未调用泛型 fn 一致）。写出 `S<Concrete>` 的类型位置（形参 / 返回 / 字段 / let / 声明 / 全局常量）由 SemaPass 复查方法体。调用点签名替换、ret / 赋值、以及实例化后的方法分派 / 内置运算符已由 SemaPass 做。其余依赖 T 具体化且未在调用点实例化的路径，throw 写 Compiler。
