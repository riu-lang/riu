...
- 缺口（sema 不报、靠 codegen）：源码从未写出 `S<Concrete>` 且无调用时，泛型 struct 方法体内依赖 T 的类型错两边都不查（与未调用泛型 fn 一致）。未调用的泛型 fn 体内嵌套调用的 E6012/E6013 同样两边都不查。写出 `S<Concrete>` 的类型位置（形参 / 返回 / 字段 / let / 声明 / 全局常量）由 SemaPass 复查方法体。调用点签名替换、ret / 赋值、声明处 Rc / Weak / Array、实例化后的方法分派 / 内置与一元运算符 / 索引（E3062）/ 成员链（E3040 / E3041 / E3152，含读路径）、以及调用点 / 实例化后的 typeArgs 推断（E6012/E6013）已由 SemaPass 做。其余依赖 T 具体化且未在调用点实例化的路径，throw 写 Compiler。
