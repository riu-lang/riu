...
- 泛型 fn/impl 体已走 SemaPass：类型参数当不透明 TypeParam。形态检查（#NoCopy / 未定义符号 / arity）在模板体报；ret / 赋值 / 方法分派 / 内置运算符在调用点 typeArgs 已知后 `substitute` 再查（E3014 / E3095 / E3001）。
...
- 缺口（sema 不报、靠 codegen）：未使用的泛型 struct 若从未写出 `S<Concrete>` / 调用，其方法体仍不查。调用点签名替换、ret / 赋值、以及实例化后的方法分派 / 内置运算符已由 SemaPass 做。其余依赖 T 具体化且未在调用点实例化的路径，throw 写 Compiler。
