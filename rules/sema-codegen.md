...
- 泛型 fn/impl 体已走 SemaPass：类型参数当不透明 TypeParam。形态检查（#NoCopy / 未定义符号 / arity）在模板体报；ret / 赋值 / 方法分派 / 内置运算符 / 一元运算符 / 索引 / 嵌套泛型推断在调用点或写出的 `S<Concrete>` 类型位置 `substitute` 后再查（E3014 / E3095 / E3001 / E3070 / E3071 / E3074 / E3062 / E6012 / E6013）。
...
- 赋值：RHS 相对存储槽 E3014。T& 局部 / `$`（Self&）store-through 比内层 T；Nullable wrap / Rc wrap / 灵活整数与 codegen 对齐。索引赋值同款。经 T& / `$` 的嵌套数组走靶向类型 E3009。索引读 / 索引赋值：subst + peelRef 后非 `[N]T` / `Array<T>` → E3062（模板形参跳过）。
