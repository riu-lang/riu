...
std::optional<ConstantValue> ConstEvaluator::evalCall(ExprCallNode* call) {
    ...
    if (!isWhitelisted(retT)) {
        throw RiuError(call->resolveLineNumber(), call->resolveColumn(), ErrorCode::E3144, fname, "return", retT.name);
    }
    auto params = header->params();
    for (auto& pn : params) {
        TypeInfo pt = pn->type() ? pn->type()->getType() : TypeInfo();
        if (!isWhitelisted(pt)) {
            throw RiuError(call->resolveLineNumber(), call->resolveColumn(), ErrorCode::E3144, fname, "parameter",
                           pt.name);
        }
    }
