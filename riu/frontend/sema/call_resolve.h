...
#ifndef RIU_LANG_SEMA_CALL_RESOLVE_H
#define RIU_LANG_SEMA_CALL_RESOLVE_H
...
//
// 从 `riu/riu/compiler/compiler_call.cpp` 抠出, 不依赖 LLVM, 用于:
...
//
// `registry` 允许为 nullptr (SemaPass 早期未持有 Riu*); 此时直接抛 E1131, 由
// Compiler 路径覆盖.
//
...
//
// `specQualified` 仅用于 E6015 hint 的人类可读名 (e.g. "riu.core.io.Writer").
...
//   - E6005 (模块文件 Riu::module 加载失败) / E6004 (模块下 fn 私有)
//   - 多重别名命中 (Package + Module 同名) 直接走 throwAmbiguousAlias
//
// `riu` 参数允许为 nullptr (SemaPass 早期可能拿不到; Compiler 路径恒非空), 此时
// 模块别名分支整段跳过 (返回 matched=false), 由 Compiler 路径兜底; 不会因 riu 缺
// 失而误报 E6005.
//
...
//
// 内建清单 (按当前 SDK assert.ut / mem.ut / ref.ut / weak.ut):
//   assert_eq, size_of, upgrade, same_ref, ptr_of, as_ref, copy_of, weak
//
// E6019：size_of typeArg 无 LLVM 布局、`__riu_reflect_type` 非已知 struct，
...
// 无 SDK 时 String / StringBuilder 乐观放行（与 riu-check 无 `; require-sdk` 对齐）。
...
// (validateFnSymbolVisibility) 重叠, 但 riu 历史给 `_ptr_offset` 这条单独分配了
...
#endif // RIU_LANG_SEMA_CALL_RESOLVE_H
