...
// 表达式语义检查：visitExpr 经 accept 分派到 visitX；visitExprList。
...
// Phase B：SemaPass 为 getType 诊断的权威抛出点。默认重抛所有 RiuError。
...
// 非 RiuError 在 debug 下 assert，禁止静默吞。
...
[[noreturn]] void rethrowGetType(ExprNode* n, RiuError e) {
...
void SemaPass::rejectEscapingRefCaptureLambda(ExprNode* expr) {
    ...
    if (auto* lam = dynamic_cast<LambdaExprNode*>(expr)) {
        if (lam->hasRefCapture()) {
            throw RiuError(lam->getLineNumber(), lam->getColumn(), ErrorCode::E4022);
        }
    }
}

void SemaPass::writeResolved(ExprNode* n, const std::function<TypeInfo()>& compute) {
    try {
        TypeInfo t = compute();
        ...
        const bool methodPointE3095 = isMethodPointCall(n) && e.getCode() && std::string_view(e.getCode()) == "E3095";
        const bool swallow =
            methodPointE3095 || (!_currentTypeParams.empty() && !isMorphologicalGenericCode(e.getCode()));
        if (!swallow) rethrowGetType(n, e);
        if (methodPointE3095) _deferredMethodE3095 = e;
    } catch (...) { // NOLINT(bugprone-empty-catch) — release 仍吞非 RiuError；debug 下 assert
#ifndef NDEBUG
        assert(false && "getType threw non-RiuError; SemaPass must not swallow unknown failures");
#endif
    }
}

void SemaPass::finishCtrlResolved(ExprNode* n, const std::function<TypeInfo()>& compute) {
    writeResolved(n, compute);
    if (!_visitExpected || !n->hasResolvedType()) return;
    TypeInfo want = _visitExpected->peelRef();
    if (!want.isArrayGeneric()) return;
    auto got = n->resolvedType();
    if (isEmptyArrayType(got) || got.isArray()) n->setResolvedType(want);
}

SemaPass::ExprVisitFrame::ExprVisitFrame(SemaPass& pass, const TypeInfo* expected, bool callCallee)
    : p(pass), prevExpected(pass._visitExpected), prevCallCallee(pass._visitCallCallee),
      prevDeferred(std::move(pass._deferredMethodE3095)) {
    p._visitExpected = expected;
    p._visitCallCallee = callCallee;
    p._deferredMethodE3095.reset();
}

SemaPass::ExprVisitFrame::~ExprVisitFrame() {
    p._visitExpected = prevExpected;
    p._visitCallCallee = prevCallCallee;
    p._deferredMethodE3095 = std::move(prevDeferred);
}

void SemaPass::visitExpr(ExprNode* expr, const TypeInfo* expected, bool callCallee) {
    if (!expr) return;
    ExprVisitFrame frame(*this, expected, callCallee);
    ...
    // 按节点类型分派；漏 override 编不过。靶向字面量的 early-return 仍在上方。
    expr->accept(*this);
}

void SemaPass::visitLiteral(ExprLiteralNode& node) {
    auto* n = &node;
    ...
    if (auto obj = dynamic_cast<LiteralObjNode*>(n->literal())) {
        if (obj->getValue().getText() == "$" && _currentFn && _currentFn->header()->isStatic()) {
            throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E3128);
        }
        ...
        if (_currentFn && obj->getValue().getText() != "$") {
            ...
            if (_movedVars.count(varName)) {
                ...
                if (!_currentLambda) {
                    throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E4033, varName);
                } else {
                    bool isParam = false;
            ...
            if (!isParam) {
                ...
                if (sym) {
                    ...
                    if (isVarOrParam && t.isHeap()) {
                        ...
                        throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E4024, elemName, varName,
                                       elemName);
                    }
                    ...
                    if (sym->kind == SymbolKind::Variable && isOuterLocalCapture(lookupFrom, sym, varName)) {
                        ...
                        if (!isCurrentTypeParam(capTy)) {
                            ...
                            if (kind == LambdaCapKind::Unsupported) {
                                throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E2029, varName,
                                               capTy.name);
                            }
    ...
    if (auto objSym = dynamic_cast<LiteralObjNode*>(n->literal())) {
        ...
        if (varName != "$") {
            ...
            if (sym && !_visitCallCallee && !_inDotBase) {
                if (symbolIsPathPrefix(sym)) {
                    throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E5016, varName);
                }
                if (sym->kind == SymbolKind::Struct) {
                    throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E5017, varName);
                }
            }
        }
    }
...
void SemaPass::visitAddSub(ExprAddSubNode& node) {
    auto* n = &node;

    visitExpr(n->left());
    visitExpr(n->right());
    writeResolved(n, [&] { return sema::typeOfAddSub(n); });
...
void SemaPass::visitMulDivMod(ExprMulDivModNode& node) {
    auto* n = &node;

    visitExpr(n->left());
    visitExpr(n->right());
    writeResolved(n, [&] { return sema::typeOfMulDiv(n); });
    string m;
    switch (n->op()) {
...
void SemaPass::visitBinOp(ExprBinOpNode& node) {
    auto* n = &node;

    visitExpr(n->left());
    visitExpr(n->right());
    writeResolved(n, [&] { return sema::typeOfBinOp(n); });
    string m;
    switch (n->op()) {
...
void SemaPass::visitCompare(ExprCompareNode& node) {
    auto* n = &node;

    visitExpr(n->left());
    visitExpr(n->right());
    writeResolved(n, [&] { return sema::typeOfCompare(n); });
...
void SemaPass::visitParen(ExprParenNode& node) {
    auto* n = &node;
...
void SemaPass::visitCall(ExprCallNode& node) {
    auto* n = &node;
    ...
    if (hasTypeArgs) {
        ...
        } else if (baseOk && !baseType.name.empty() && !isBuiltinType(baseType.name) && !baseType.isArrayGeneric() &&
    ...
    if (!n->getTypeArgs().empty()) {
        ...
        try {
            ...
            if ((calleeName == "rc" || calleeName == "Rc" || calleeName == "Weak" || calleeName == "Array") &&
                ...
                throw RiuError(eline, ecol, ErrorCode::E4025, calleeName, inner ? inner->name : std::string("?"));
            }
            if ((calleeName == "rc" || calleeName == "Rc" || calleeName == "Weak") && t0.isDyn()) {
                throw RiuError(eline, ecol, ErrorCode::E1132, calleeName + "<" + t0.getFullName() + ">");
            }
            for (auto& tn : n->getTypeArgs()) {
                try {
                    ...
                    throw;
                } catch (...) { // NOLINT(bugprone-empty-catch) — sema 非 RiuError 异常留 Compiler 兜底
                }
            }
        } catch (const RiuError&) {
            throw;
    ...
    if (auto lit = dynamic_cast<ExprLiteralNode*>(n->getCalleeExpr())) {
        if (auto obj = dynamic_cast<LiteralObjNode*>(lit->literal())) {
                ...
                if (genFn && genFn->header()->hasAnno("Builtin")) {
                    ...
                    if (hasTypeArgs) {
                        ...
                        const bool stringOverload = fnName == "assert_eq" && argTypesOk && argTypes.size() >= 2 &&
                                                    peelStr(argTypes[0]).isString() && peelStr(argTypes[1]).isString();
                        if (!argTypesOk || stringOverload) {
                            typeArgsOk = false;
                        } else if (fnName == "as_ref" && !argTypes.empty() && argTypes[0].isHeap()) {
                            auto inner = argTypes[0].heapElementType();
                            if (!inner) {
                                throw RiuError(line, col, ErrorCode::E6029, fnName, argTypes[0].getFullName());
                            }
                    ...
                    if (typeArgsOk && !typeArgs.empty()) {
                        sema::validateBuiltinIntrinsicShape(fnName, typeArgs.size(), n->getArgs().size(), line, col);
                        if (argTypesOk) {
                            bool skipTypeShape = false;
                            if (isCurrentTypeParam(typeArgs[0]) &&
                                (fnName == "assert_eq" || fnName == "same_ref" || fnName == "ptr_of" ||
                                 fnName == "ptr_cast" || fnName == "copy_of" || fnName == "size_of" ||
                                 fnName == "align_of" || fnName == "overlay" || fnName == "__riu_reflect_type")) {
                                skipTypeShape = true;
                            }
                            ...
                            if (fnName == "copy_of" && !isCurrentTypeParam(typeArgs[0])) {
                                ...
                                if (isNoCopyTypeIn(T, _file, _sdkFile)) {
                                    throw RiuError(n->getLineNumber(), n->getColumn(), ErrorCode::E4031, T.name,
                                                   "copy_of", T.name);
                                }
                            }
            ...
            // （与未调用泛型 fn 一致，riu-check 是 riu build 的子集）.
            if (_riu && !structDecl) {
                ...
                if (genericFn && genericFn->header()->isGeneric() && !genericFn->header()->hasAnno("Builtin")) {
                    ...
                    if (typeArgsOk && typeArgs.size() == genericFn->header()->typeParams().size()) {
                        ...
                        sema::validateGenericTypeArgsSpecBound(&_riu->specRegistry(), &_riu->specImplChecker(), fnOwner,
                                                               genericFn->header(), typeArgs, line, col);
    ...
    if (auto dotCallee = dynamic_cast<ExprDotNode*>(n->getCalleeExpr())) {
        ...
        if (dotCallee->hasSpecQualifier()) {
            ...
            if (baseOk && !baseType.name.empty()) {
                ...
                if (baseType.isDyn()) {
                    if (_riu) {
                        const SpecRegistry* reg = &_riu->specRegistry();
                        ...
                        if (resolvedDyn.decl && resolvedDyn.decl->name().getText() != specName) {
                            throw RiuError(dline, dcol, ErrorCode::E1101, baseType.name, specName, memberName);
                        }
                    }
                    ...
                    if (_riu) {
                        auto resolved = _riu->specRegistry().resolve(specName, _file);
                    ...
                    if (specDecl) {
                        ...
                        if (sigIdx == kNoIdx) {
                            throw RiuError(dline, dcol, ErrorCode::E1140, specName, memberName,
        ...
        if (!dotCallee->hasSpecQualifier()) {
            if (dotCallee->member() == "inv") {
                try {
                    ...
                    if (invBase.isFloat()) {
                        throw RiuError(n->getLineNumber(), n->getColumn(), ErrorCode::E3070, invBase.name);
                    }
                } catch (const RiuError&) {
                    throw;
                } catch (...) { // NOLINT(bugprone-empty-catch)
            ...
            if (isField) {
                ...
                if (tyOk && !fieldTy.empty() && !isFnCalleeType(fieldTy, _file, _sdkFile)) {
                    throw RiuError(n->getLineNumber(), n->getColumn(), ErrorCode::E3095, fieldTy.getFullName());
                }
            }
        }
        vector<TypeInfo> argTypes;
        ...
        if (ok) {
            auto modCall = sema::resolveModuleFnCall(_file, _riu, n, dotCallee, argTypes);
            if (!modCall.matched) {
                ...
                if (baseOk) {
                    ...
                    if (baseType.isArrayGeneric()) {
                        ...
                        sema::resolveDynMethodSig(resolved.decl, resolved.qualified, baseType, member, argTypes, dline,
                                                  dcol);
                    }
                }
            }
                    ...
                    try {
                        ...
                        if (expectedParams[idx] && !argType.name.empty() && !argType.isSelf() &&
                            ...
                            if (argType.isRef() && !expectedParams[idx]->isRef()) {
                                ...
                                throw RiuError(n->getLineNumber(), n->getColumn(), ErrorCode::E3014,
    ...
    // struct 方法私有可见性检查（E6007）——因 riu-check 不跑 LLVM
    ...
    if (auto dotCallee = dynamic_cast<ExprDotNode*>(n->getCalleeExpr())) {
        try {
            ...
            if (isCurrentTypeParam(baseType)) {
                ...
                sema::resolveMethodOverload(_file, _sdkFile, baseType.name, member, n->getArgs(), n->getLineNumber());
    ...
    if (_deferredMethodE3095) {
        ...
        if (dot && !dot->hasSpecQualifier()) {
            ...
            try {
                baseType =
                    dot->baseExpr()->hasResolvedType() ? dot->baseExpr()->resolvedType() : dot->baseExpr()->getType();
                baseType = applyInstSubst(baseType.peelAutoDeref());
            ...
            if (!(baseOk && isCurrentTypeParam(baseType))) {
                if (baseOk && dot->member() == "inv" && baseType.isFloat()) {
                    throw RiuError(n->getLineNumber(), n->getColumn(), ErrorCode::E3070, baseType.name);
                }
                bool isField = false;
                try {
...
void SemaPass::visitDot(ExprDotNode& node) {
    auto* n = &node;
        ...
        if (mem == "type_info" || mem == "fields" || mem == "methods" || mem == "variants") {
            ...
            if (bt.kind == TypeKind::Normal && !bt.name.empty()) {
                ...
                if (sd && sd->fieldIndex(mem) < 0) {
                    throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E1138, mem, bt.name, bt.name,
                                   mem);
                }
            }
        }
    ...
    // 点链首段既是路径前缀又是局部值：调用与取值都 E5015（不靠 _visitCallCallee）。
    ...
    if (!_inDotBase && _file) {
        ...
        if (ExprDotNode::parseChain(n, aliasName, segs)) {
            ...
            if (symbolIsPathPrefix(pathSym) && hasLocalValueNamed(n, aliasName)) {
                throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E5015, aliasName,
                               string("path prefix and local value"));
            }
            if (!_visitCallCallee) {
                if (pathSym && pathSym->kind == SymbolKind::Module) {
                    FileNode* target = _file->moduleAlias(aliasName);
                    if (!target && _riu) target = _riu->module(pathSym->moduleName);
                    if (target && segs.size() == 1) {
                        const string& mem = segs[0];
                        if (hasPublicTypeIn(target, mem)) {
                            throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E5017,
                                           aliasName + "." + mem);
                        }
                        if (!target->lookupFnSymbol(mem)) {
                            throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E5016, aliasName);
                        }
                    }
                } else if (pathSym && pathSym->kind == SymbolKind::Package) {
                    if (segs.size() == 1) {
                        if (_riu) {
                            (void)_riu->resolvePkgPath(_file, segs[0], n->resolveLineNumber(), pathSym->moduleName);
                        }
                        string dotted = aliasName + "." + segs[0];
                        throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E5016, dotted);
                    }
                    string childKey;
                    ...
                    if (_riu) {
                        (void)_riu->resolvePkgPath(_file, childKey, n->resolveLineNumber(), pathSym->moduleName);
                    }
    ...
    if (!_visitCallCallee) {
        ...
        if ((skipField || isMethod) && !mem.starts_with("to_") && !skipPkg) {
            ...
            if (!isField) {
                throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E3090);
            }
        }
    }
    return;
...
void SemaPass::visitIfElse(ExprIfElseNode& node) {
    auto* n = &node;
    ...
    visitBlock(n->thenBlock(), _visitExpected);
    ...
    for (auto& el : n->elifs()) {
        ...
        visitBlock(el->block(), _visitExpected);
    ...
    if (n->elseBlock()) {
        visitBlock(n->elseBlock(), _visitExpected);
...
void SemaPass::visitOneLineIfElse(ExprOneLineIfElseNode& node) {
    auto* n = &node;

    visitExpr(n->condition());
    visitExpr(n->trueValue(), _visitExpected);
    visitExpr(n->falseValue(), _visitExpected);
    finishCtrlResolved(n, [&] { return sema::typeOfOneLineIfElse(n); });
    tryValidateOneLineIfElse(n);
    return;
}

void SemaPass::visitGet(ExprGetNode& node) {
    auto* n = &node;
    ...
    writeResolved(n, [&] { return sema::typeOfGet(n); });
    tryValidateIndexBase(n->arrayExpr(), n->resolveLineNumber(), n->resolveColumn());
    return;
}

void SemaPass::visitArray(ExprArrayNode& node) {
    auto* n = &node;
    ...
    checkEmptyArrayLiteral(n, _visitExpected);
    writeResolved(n, [&] { return sema::typeOfArray(n); });
    return;
}

void SemaPass::visitTuple(ExprTupleNode& node) {
    auto* n = &node;
    ...
    try {
        ...
        if (!typeStillTemplate(tt) && !sema::typeHasLlvmLayout(tt, _file, _sdkFile, _currentTypeParams)) {
            throw RiuError(n->getLineNumber(), n->getColumn(), ErrorCode::E3098, tt.name, string("(tuple)"),
                           string("(tuple)"));
        }
    } catch (const RiuError&) {
        throw;
...
void SemaPass::visitUnary(ExprUnaryNode& node) {
    auto* n = &node;

    visitExpr(n->right());
    if (n->op() == ExprUnaryNode::Op::Neg) applyUnaryNegToIntLits(n->right());
    writeResolved(n, [&] { return sema::typeOfUnary(n); });
    string m;
    switch (n->op()) {
...
void SemaPass::visitLambda(LambdaExprNode& node) {
    auto* n = &node;
            ...
            if (lastBare && bodyExp) {
                ...
                if (auto ma = dynamic_cast<ExprMoveAssignNode*>(se->expr())) {
                    DiagnosticEngine::emit(_sourcePath,
                                           RiuError(ma->resolveLineNumber(), ma->resolveColumn(), ErrorCode::E4030));
                }
    ...
    if (_currentLambdaHasRefCapture && _currentLambdaHasHandleCapture) {
        throw RiuError(n->getLineNumber(), n->getColumn(), ErrorCode::E2029, _currentLambdaHandleCapName,
...
void SemaPass::visitStructLit(ExprStructLitNode& node) {
    auto* n = &node;
    ...
    if (n->isSelfForm()) {
        ...
        auto r = sema::resolveExprTypeLhs(_file, _riu, n->typePath(), line, col);
        structTy = r.type;
    ...
    if (n->positional()) {
        ...
        if (isNoCopyTypeIn(f->getType(), _file, _sdkFile)) {
            if (!isFreshHandleExpr(n->positional())) {
                throw RiuError(line, col, ErrorCode::E4031, f->getType().name, "struct 字面量字段初始化",
                               f->getType().name);
            }
        }
        return;
    ...
    for (auto& fi : n->fields()) {
        ...
        if (decl->fieldIndex(fname) < 0) {
            throw RiuError(fline, fcol, ErrorCode::E3126, structName, fname);
        }
        if (!seen.insert(fname).second) {
            throw RiuError(fline, fcol, ErrorCode::E3127, fname);
        }
        TypeInfo fieldExpected;
        const TypeInfo* fieldExpPtr = nullptr;
        if (!decl->isGeneric()) {
            ...
            if (fieldIdx >= 0) {
                ...
                if (isNoCopyTypeIn(fieldType, _file, _sdkFile)) {
                    if (!isFreshHandleExpr(fi->value())) {
                        throw RiuError(fline, fcol, ErrorCode::E4031, fieldType.name, "struct 字面量字段初始化",
                                       fieldType.name);
                    }
                }
            }
...
void SemaPass::visitPathCall(ExprPathCallNode& node) {
    auto* n = &node;
    ...
    if (selfForm && lhsName == "Self") {
        throw RiuError(n->getLineNumber(), n->getColumn(), ErrorCode::E3123);
    }
    ...
    // 优先于 impl-method / enum-ctor 分流 (struct 无需 impl 也能取反射元数据).
    {
        ...
        if (n->args().empty() &&
            ...
            if (sd) {
                ...
                if (rhsName == "variants") {
                    throw RiuError(n->getLineNumber(), n->getColumn(), ErrorCode::E3135, lhsName);
                }
                ...
                if (rhsName == "fields") {
                    ...
                    if (!hasInst) {
                        throw RiuError(n->getLineNumber(), n->getColumn(), ErrorCode::E6019,
    ...
    // struct/enum 重名在 riu 里非法 (E2017), 此处直接按 lhsName 查 struct 优先.
    {
        ...
        if (n->args().empty()) {
            ...
            if (isCurrentTypeParam(lhsTy)) {
                auto fieldType =
                    typeParamBoundStaticFieldType(_currentFn, _file, _sdkFile, lhsTy.name, n->variantName().getText());
                if (fieldType) {
                    if (n->errPropagate()) {
                        throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E7001);
                    }
        ...
        if (structImpl) {
            ...
            int col = n->resolveColumn();
            if (!methodHeader) {
                throw RiuError(line, col, ErrorCode::E3121, lhsName, rhsName);
            }
            if (!methodHeader->isStatic()) {
                throw RiuError(line, col, ErrorCode::E3120, lhsName, rhsName, rhsName);
            }
            ...
            if (structDecl && !structDecl->isGeneric() && !n->lhsTypeArgs().empty()) {
                throw RiuError(line, col, ErrorCode::E6011, lhsName, static_cast<size_t>(0), n->lhsTypeArgs().size())
            ...
            if (!skipTypeCheck) {
                ...
                if (paramTypesOk) {
                    ...
                    const map<string, TypeInfo>* cmpSubst = !staticSubst.empty() ? &staticSubst : currentInstSubst();
                    // arity 校验
                    if (n->args().size() != paramTypes.size()) {
                        ...
                        throw RiuError(line, col, ErrorCode::E3131, lhsName, rhsName, paramTypes.size(), expected,
                                       n->args().size(), got);
                    }
                    // 类型逐位比对
                    vector<TypeInfo> argTypes;
                        ...
                        for (size_t i = 0; i < argTypes.size(); ++i) {
                            ...
                            if (!(argTypes[i] == paramTypes[i])) {
                                ...
                                if (!nullableMatch) {
                                    ...
                                    if (paramTypes[i].isRef() && argIsRefIdentMatching(n->args()[i], paramTypes[i])) {
                                        continue;
                                    }
                                    throw RiuError(line, col, ErrorCode::E3131, lhsName, rhsName, paramTypes.size(),
                    ...
                    for (size_t i = 0; i < n->args().size() && i < paramTypes.size(); ++i) {
                        ...
                        if (isNoCopyTypeIn(pt, _file, _sdkFile)) {
                            if (!isFreshHandleExpr(n->args()[i])) {
                                throw RiuError(line, col, ErrorCode::E4031, pt.name, "按值传参", pt.name);
                            }
                        }
                    }
                }
...
void SemaPass::visitMatch(ExprMatchNode& node) {
    auto* n = &node;
    ...
    for (auto& arm : n->arms()) {
        if (arm->hasBlock())
            ...
            visitExpr(arm->body(), _visitExpected);
    }
...
void SemaPass::visitTryCatch(ExprTryCatchNode& node) {
    auto* n = &node;
    ...
    for (auto& arm : n->catches()) {
        ...
        if (!enumDecl) {
            ...
            throw RiuError(aline, acol, ErrorCode::E7011, arm->errName().getText(), arm->errType(), arm->errType());
        }
        catchTypes.push_back(arm->errType());
    }

    _tryStack.emplace_back();
    visitBlock(n->tryBlock(), _visitExpected);
    ...
    for (auto& seen : seenErrTypes) {
        ...
        if (!covered) {
            throw RiuError(line, col, ErrorCode::E7002, seen, string("<unknown>"), seen);
        }
    }

    for (auto& c : n->catches())
        visitBlock(c->body(), _visitExpected);
    ...
    if (!blockTerminatesFlow(trySc, n->tryBlock()) && n->tryBlock()->hasResult() && n->tryBlock()->resultExpr()) {
        ...
        if (tryGetExprType(n->tryBlock()->resultExpr(), resultType)) {
            ...
            for (auto& arm : n->catches()) {
                ...
                int aline = arm->getLineNumber() > 0 ? arm->getLineNumber() : line;
                int acol = arm->getColumn() > 0 ? arm->getColumn() : col;
                throw RiuError(aline, acol, ErrorCode::E7010, armT.name, resultType.name);
            }
        }
    }
    return;
...
void SemaPass::visitDynCtor(ExprDynCtorNode& node) {
    auto* n = &node;
    ...
    // 仅在 _riu 就绪时校验 (spec 注册表 + impl 检查器都从 Riu 取); SDK 自构建
    ...
    try {
        ...
        if (!specDecl) {
            throw RiuError(line, col, ErrorCode::E1131, specBareName.empty() ? string("?") : specBareName);
        }
        if (specInner && specInner->isDyn()) {
            throw RiuError(line, col, ErrorCode::E1132, resultType.getFullName());
        }
        auto& checker = _riu->specImplChecker();
        if (!checker.specIsObjectSafe(specDecl)) {
            throw RiuError(line, col, ErrorCode::E1134, specQualified, specQualified, specQualified);
        }
        ...
        if (concreteBare.empty()) {
            throw RiuError(line, col, ErrorCode::E1133, specQualified, argType.getFullName(), specQualified);
        }
...
void SemaPass::visitNullElse(ExprNullElseNode& node) {
    auto* n = &node;

    visitExpr(n->left());
    // 右侧按左侧 Nullable 内层靶向：`a ?? []` 的 `[]` 须是 Array<T>，否则 E3063。
    // 与 compileMoveAssignExpr / if 块值同一套 visitExpr(..., _visitExpected)。
    TypeInfo inner;
    ...
    try {
        ...
        if (!leftType.isNullable()) {
            if (!leftType.name.empty()) {
                throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E3024, leftType.name);
            }
            return;
        }
        ...
        if (!(rightType == inner)) {
            throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E3014, inner.name, rightType.name);
        }
    } catch (const RiuError&) {
        throw;
    } catch (...) { // NOLINT(bugprone-empty-catch)
...
void SemaPass::visitMoveAssign(ExprMoveAssignNode& node) {
    auto* n = &node;
    ...
    if (!isMoveAssignLvalue(n->left())) {
        throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E4036);
    }
    ...
    try {
        ...
        if (leftType != rightType) {
            ...
            if (isBuiltinType(leftType.name)) {
                throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E3001, "arithmetic",
...
// Phase 3.4.d.1: ExprGetRefNode —— 无子表达式可递；1.7b 在本分支经
// typeOfGetRef 写槽（E3030/E3040/E3041/E3043/E3097 由 writeResolved 重抛）。
// compileGetRefExpr 的内联 throw 在正常 codepath 下不可达。
// Phase 3.4.d.2: 补 E3042 链式私有字段可见性校验.
void SemaPass::visitGetRef(ExprGetRefNode& node) {
    auto* n = &node;

    writeResolved(n, [&] { return sema::typeOfGetRef(n); });
    ...
    auto checkInlineConst = [&](FileNode* f) {
        ...
        for (const auto& gc : f->getGlobalConsts()) {
            if (gc->name().getText() == n->obj().getText() && gc->isInline()) {
                throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E3118, n->obj().getText());
            }
        }
    };
...
void SemaPass::visitArrayInit(ExprArrayInitNode& node) {
    auto* n = &node;
