...
void SemaPass::visitExpr(ExprNode* expr, const TypeInfo* expected, bool callCallee) {
    ...
    std::optional<RiuError> deferredMethodE3095;
                ...
                if (!isParam) {
                    ...
                    if (sym) {
                        ...
                        if (isVarOrParam && t.isHeap()) {
                            ...
                            throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E4024, elemName,
                                           varName, elemName);
                        }
                        ...
                        if (sym->kind == SymbolKind::Variable && isOuterLocalCapture(lookupFrom, sym, varName)) {
                            ...
                            if (!isCurrentTypeParam(capTy)) {
                                ...
                                if (kind == LambdaCapKind::Unsupported) {
                                    throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E2029,
                                                   varName, capTy.name);
                                }
                ...
                if (_riu && !structDecl) {
                    ...
                    if (genericFn && genericFn->header()->isGeneric() && !genericFn->header()->hasAnno("Builtin")) {
                        ...
                        if (typeArgsOk && typeArgs.size() == genericFn->header()->typeParams().size()) {
                            ...
                            sema::validateGenericTypeArgsSpecBound(&_riu->specRegistry(), &_riu->specImplChecker(),
            ...
            if (mem == "type" || mem == "fields" || mem == "methods" || mem == "variants") {
                ...
                if (bt.kind == TypeKind::Normal && !bt.name.empty()) {
                    ...
                    if (sd && sd->fieldIndex(mem) < 0) {
                        throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E1138, mem, bt.name,
                                       bt.name, mem);
                    }
                }
            }
                ...
                if (lastBare && bodyExp) {
                    ...
                    if (auto ma = dynamic_cast<ExprMoveAssignNode*>(se->expr())) {
                        DiagnosticEngine::emit(
                            _sourcePath, RiuError(ma->resolveLineNumber(), ma->resolveColumn(), ErrorCode::E4030));
                    }
        ...
        // struct/enum 重名在 riu 里非法 (E2017), 此处直接按 lhsName 查 struct 优先.
        {
            ...
            if (structImpl) {
                ...
                if (structDecl && !structDecl->isGeneric() && !n->lhsTypeArgs().empty()) {
                    throw RiuError(line, col, ErrorCode::E6011, lhsName, static_cast<size_t>(0),
                                   n->lhsTypeArgs().size())
    ...
    if (auto n = dynamic_cast<ExprNullElseNode*>(expr)) {
        ...
        try {
            ...
            if (!(rightType == inner)) {
                throw RiuError(n->resolveLineNumber(), n->resolveColumn(), ErrorCode::E3014, inner.name,
                               rightType.name);
            }
        } catch (const RiuError&) {
            throw;
