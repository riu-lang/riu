...
public:
    ...
    // match arm 绑定：builder 只注册空槽；visitMatch 下钻臂体前按 scrut 填
    // SymbolInfo.type。getType / structuralType 若更早碰到 match，惰性填一次
    // （已填则跳过）。找不到 enum/variant 则保持空槽，E2019/E2020 仍由
    // tryValidateMatchScrut 抛。
