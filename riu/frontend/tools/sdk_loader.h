...
//   main.cpp 自己 catch + reportRuntimeError + exit; riu-check 走 renderRiuError。
...
// 定位 SDK 目录 (返回 .../sdk/riu/src/riu/core), 找不到返回 "". Windows 专用。
...
// allowDecl：true 时优先读 .ud（依赖方只要接口 + 泛型体）；false 时整文件 parse。
// forceFullParseAbs：这些源文件绝对路径（generic_string）跳过 .ud，整文件 parse
// （SDK 自构建、对应 .obj 过期，需要函数体 codegen）。其余模块仍可读 .ud。
