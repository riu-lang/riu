...
// 文件首行为编译器指纹（riu.exe mtime + size），重编 riu 即整文件作废；
// 后续每行 `<filename>\t<src-mtime-ns>\t<src-size>` 记录该目录中单个 .ut 源文件的元信息，
...
#ifndef RIU_LANG_PKG_CACHE_H
#define RIU_LANG_PKG_CACHE_H
...
// 计算当前 riu.exe 的指纹（mtime_ns + size，TAB 分隔）。
...
//   `<projectRoot>/src/A/B/foo.ut` -> `<buildDir>/src/A/B/foo`
// 调用方再拼 `.obj` / `.ll`。
