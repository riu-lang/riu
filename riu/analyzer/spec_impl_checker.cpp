...
// 内置类型按 riu.core 归属 (§12.5 orphan 用; SDK base.ut 内为这些类型
...
SpecImplChecker::SpecImplChecker(Riu* riu) : _riu(riu) {}
...
void SpecImplChecker::buildTypeOwnerMap() {
    ...
    if (_riu) {
        if (auto sdk = _riu->sdkFile()) index(sdk);
...
std::string SpecImplChecker::moduleOfType(const std::string& bareName) const {
    ...
    if (builtinTypeNames().count(bareName)) return "riu.core";
    return {};
}
...
void SpecImplChecker::validate() {
    ...
    // `riu test` 冷启动 — compileSdkDir 触发第一次 validate, 此时 test 文件还未
    ...
    // （base.ut / assert.ut 等）中。收集到 processedSet 去重，避免 SDK 编译
    // 时 _riu->files() 与 sdk->wildcardImports() 重叠导致 E1103 误报。
...
void SpecImplChecker::checkExplicitImplicitConflict() {
    ...
    if (auto sdk = _riu->sdkFile()) collect(sdk);
    for (auto& f : _riu->files())
        collect(f);
        ...
        for (auto& [pf, pm] : slot.plain) {
            for (auto& [df, dm, dq] : slot.draft) {
                ...
                if (sigEquivalent(pm, dm, {})) {
                    throw RiuError(pm->getLineNumber(), pm->getColumn(), ErrorCode::E1105, pm->name().getText(),
                                   typeQualified, dq);
                }
            }
        }
...
void SpecImplChecker::validateImpl(FileNode* implFile, StructImplNode* impl) {
    ...
    for (auto& dref : impl->specRefs()) {
        ...
        if (!resolved) {
            ...
            throw RiuError(dref.line, dref.col, ErrorCode::E3030, dref.name);
        }
        ...
        if (!implMod.empty() && !typeLocalToImplFile && implMod != typeOwnerMod && implMod != specOwnerMod) {
            throw RiuError(impl->getLineNumber(), impl->getColumn(), ErrorCode::E1120, specQualified, typeQualified);
        }
        ...
        for (auto& required : draft->staticFields()) {
            ...
            if (!provided || provided->type->getType() != requiredType || !qualifiersMatch) {
                throw RiuError(impl->getLineNumber(), impl->getColumn(), ErrorCode::E1141, typeQualified, fieldName,
                               specQualified);
            }
        }
    ...
    for (auto& [groupKey, entries] : sigGroups) {
        ...
        if (withDefault.empty()) {
            ...
            throw RiuError(impl->getLineNumber(), impl->getColumn(), ErrorCode::E1101, typeQualified, e.specQualified,
                           name);
        }
        ...
        if (withDefault.size() >= 2) {
            ...
            throw RiuError(impl->getLineNumber(), impl->getColumn(), ErrorCode::E3132, typeQualified, name, specList);
        }
...
//   - 类型字面量名 == "Self"  (riu 当前无 Self 关键字, 但保留语义层禁用)
//   - 类型字面量名 == draft 自身名 (例如 `draft D { fn clone() D }`)
//
// "非 receiver" 位: riu fnSig 不显式承载 receiver, 故所有 params + retType
...
void SpecImplChecker::validateDynTypeReferences() {
    ...
    // 去重：SDK flat 文件可能同时出现在 sdk->wildcardImports() 和 _riu->files() 中
    set<FileNode*> processedSet;
    auto runOnce = [&](FileNode* file) {
        if (!file || !processedSet.insert(file).second) return;
        run(file);
...
void SpecImplChecker::validateDynInTypeNode(TypeNode* tn, FileNode* file, const std::string& outerWrapper) const {
    ...
    if (auto* gen = dynamic_cast<TypeGenericNode*>(tn)) {
        ...
        if (t.isDyn() && gen->typeArgs().size() == 1) {
            ...
            if (outerWrapper == "Rc" || outerWrapper == "Weak" || outerWrapper == "Dyn") {
                throw RiuError(line, col, ErrorCode::E1132, outerWrapper + "<" + t.getFullName() + ">");
            }
            if (outerWrapper == "Nullable") {
                throw RiuError(line, col, ErrorCode::E1135);
            }
            ...
            if (auto* innerGen = dynamic_cast<TypeGenericNode*>(innerStripped)) {
                if (innerGen->getType().isDyn()) {
                    throw RiuError(line, col, ErrorCode::E1132, t.getFullName());
                }
            }
            ...
            if (!specDecl) {
                throw RiuError(line, col, ErrorCode::E1131, specBare.empty() ? std::string("?") : specBare);
            }
    ...
    if (auto* normal = dynamic_cast<TypeNormalNode*>(tn)) {
        ...
        if (!alias && _riu && _riu->sdkFile() && _riu->sdkFile() != file) {
            alias = _riu->sdkFile()->getAliasDecl(name);
        }
