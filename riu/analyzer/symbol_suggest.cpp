...
vector<string> SymbolSuggest::nearby(ScopeNode* scope, const string& target, int maxDist, int k) {
    ...
    std::ranges::sort(scored, [](const pair<int, string>& a, const pair<int, string>& b) {
...
string SymbolSuggest::buildHint(ScopeNode* scope, const string& target, int maxDist, int k) {
...
void SymbolSuggest::throwSymbolNotFound(ScopeNode* scope, int line, int col, const ErrorCodeDef& ec,
                                        const string& name) {
    auto err = RiuError(line, col, ec, name);
