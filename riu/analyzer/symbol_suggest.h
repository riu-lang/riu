...
#ifndef RIU_LANG_SYMBOL_SUGGEST_H
#define RIU_LANG_SYMBOL_SUGGEST_H
...
vector<string> nearby(ScopeNode* scope, const string& target, int maxDist = 2, int k = 3);

// 一步到位：直接构造 hint 字符串（无候选返回空串）
// 单候选："did you mean `foo`?"
// 多候选："did you mean one of: `foo`, `bar`?"
string buildHint(ScopeNode* scope, const string& target, int maxDist = 2, int k = 3);

// 抛"符号未找到"诊断的便利函数：自动附拼写近似 hint（无候选时不附）
// 用于 E3030 / E3031 / E3032 等单参数错误码的统一抛点
[[noreturn]] void throwSymbolNotFound(ScopeNode* scope, int line, int col, const ErrorCodeDef& ec, const string& name);
} // namespace SymbolSuggest

#endif // RIU_LANG_SYMBOL_SUGGEST_H
