...
#ifndef RIU_LANG_BUILTIN_METHODS_H
#define RIU_LANG_BUILTIN_METHODS_H
...
// String 方法当前是 riu 实现，表里占 BuiltinRecv::String，无行。
namespace sema {
...
inline void validateBuiltinMethodCall(const BuiltinMethodSpec& spec, const TypeInfo& recv, size_t argsCount,
    ...
    if (spec.needsElemType) {
        if (!recv.arrayGenericElementType() && !recv.elementType) {
            throw RiuError(line, col, ErrorCode::E3050);
        }
    }
    if (spec.needsLvalue && !baseIsLvalue) {
        throw RiuError(line, col, ErrorCode::E6042, spec.name);
    }
    if (argsCount != static_cast<size_t>(spec.arity)) {
        throw RiuError(line, col, ErrorCode::E6027, spec.name, static_cast<size_t>(spec.arity));
    }
}

} // namespace sema

#endif // RIU_LANG_BUILTIN_METHODS_H
