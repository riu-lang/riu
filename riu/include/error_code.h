...
//   throw RiuError(line, col, ErrorCode::E3001, leftType.name, rightType.name);
...
#ifndef RIU_LANG_ERROR_CODE_H
#define RIU_LANG_ERROR_CODE_H
...
DEF_ERR(2003, "module `{}` is ambiguous: both `{}.ut` and `{}/` exist")
...
DEF_ERR(2014, "`#Test` is only allowed in `*.test.ut` files; `{}` is not a test file")
...
// ── E5xxx 模块 / 包 ───────────────────────────────────────────────────
DEF_ERR(5001, "riu.toml not found in {}")
DEF_ERR(5002, "riu.toml is missing required field `name`")
DEF_ERR(5003, "riu.toml field `name` must be a string")
DEF_ERR(5004, "riu.toml field `name` must not be empty")
DEF_ERR(5005, "riu.toml must declare `[library]` or `[[executable]]`")
DEF_ERR(5006, "riu.toml `[library].type` must be \"static\" or \"dynamic\"")
DEF_ERR(5007, "riu.toml `[library].type=\"dynamic\"` not yet supported")
DEF_ERR(5008, "riu.toml output name `{}` is used more than once")
DEF_ERR(5009, "failed to parse riu.toml: {}")
DEF_ERR(5010, "syntax errors in {}")
DEF_ERR(5011, "circular module import: {}")
DEF_ERR(5012, "module not found: {} (expected file {})")
DEF_ERR(5013, "riu.toml `entry` must be a relative path under `src/`, got absolute path: {}")
DEF_WARN(5014, "riu.toml `entry` resolves outside `src/` (`{}`): convention is that all sources live under `src/`; obj "
...
// riu.toml 多产物（#22）。E5005–E5008 旧文案作废。
DEF_ERR(5021, "riu.toml no longer accepts `{}`; use `[library]`, `[[executable]]`, or `external_links`")
DEF_ERR(5022, "riu.toml `[library]` must be a table")
DEF_ERR(5023, "riu.toml `[[executable]]` must be an array of tables")
DEF_ERR(5024, "riu.toml `[library]` is missing required field `lib_mod`")
DEF_ERR(5025, "riu.toml `[[executable]]` is missing required field `entry`")
DEF_ERR(5026, "riu.toml `external_links` path must start with `//` or `./`: `{}`")
DEF_ERR(5027, "riu.toml `external_links` must not include a library suffix: `{}`")
DEF_ERR(5028, "riu.toml product name `{}` is not allowed")
DEF_ERR(5029, "riu.toml is missing required field `version`")
DEF_ERR(5030, "riu.toml field `version` must be a non-empty string")
...
DEF_ERR(6031,
        "assert_eq operand type mismatch: actual is '{}', expected is '{}' (riu 不做隐式整型/浮点转换；整型字面量默认 "
