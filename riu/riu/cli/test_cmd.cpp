...
[[noreturn]] void runTestCommand(const TestCmdOptions& opts) {
    ...
    if (self.empty()) {
        std::cerr << "Error: failed to resolve riu executable path\n";
        std::exit(1);
    }

    std::string cwd = fs::current_path().string();

    // 1. riu build --test（复用缓存，只重编变化的文件）
    std::cout.flush();
    ...
    std::cout.flush();
    {
        ...
        if (code != 0) {
            std::cerr << "Error: riu build --test failed (exit code " << code << ")\n";
    ...
    if (!fs::exists(testsDir)) {
        // 从 project root 找
        fs::path root = cwd;
        while (!root.empty() && !fs::exists(root / "riu.toml")) {
    ...
    // 但保留此过滤以应对极端竞态；同时避免重复统计）
    {
        ...
        for (auto& marker : failedMarkers) {
            ...
            // fname = "riu_core_array_test.test.failed" → "riu_core_array_test.test.dll"
    ...
    if (!opts.testMod.empty()) {
        // DLL 名由模块名推导：riu.core.array → riu_core_array_test.test.dll
        ...
        std::vector<std::string> filtered;
        for (auto& p : dllPaths) {
            std::string stem = fs::path(p).stem().string(); // 如 riu_core_array_test.test
    ...
    // 4. 并行 spawn riu-test-runner，输出重定向到日志文件
    ...
    if (!dllPaths.empty()) {
        std::cout.flush();
        std::string runnerExe = "riu-test-runner";
