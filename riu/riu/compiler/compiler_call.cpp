...
llvm::Function* Compiler::getMethodFunction(const string& structName, const string& methodName,
    ...
    if (auto* inst = _generic.structs().find(structName)) {
        ...
        } else if (_riu && _riu->sdkFile() && _riu->sdkFile() != _file) {
            // SDK 平铺文件拆分后，_sdkFile 空壳通过 wildcardImports 找到真实 owner
            auto* sdkOwner = _riu->sdkFile()->getStructOwner(structName);
            if (sdkOwner) {
                ownerModule = sdkOwner->moduleName();
            } else if (_riu->sdkFile()->getStructDecl(structName)) {
                ownerModule = _riu->sdkFile()->moduleName();
            }
        }
    }
    // 生成 mangle 名称
...
bool Compiler::isBuiltinMethod(const string& structName, const string& methodName) {
    if (!_riu || !_riu->sdkFile()) return false;

    auto structImpl = _riu->sdkFile()->getStructImpl(structName);
...
// riu/frontend/sema/call_resolve.cpp 的 sema::checkErrPropagateForIdCall, 形参改成
...
llvm::Value* Compiler::compileCallExpr(ExprCallNode* node) {
    ...
    if (auto litCallee = dynamic_cast<ExprLiteralNode*>(calleeExpr)) {
        if (auto objLit = dynamic_cast<LiteralObjNode*>(litCallee->literal())) {
            ...
            if (_riu && _riu->sdkFile() && _riu->sdkFile() != _file) {
                _riu->sdkFile()->collectFnOverloads(fnName, cands);
            }
            ...
            string srcPath = (_riu && _file) ? _riu->modulePath(_file->moduleName()) : "";
        ...
        if (auto objLiteral = dynamic_cast<LiteralObjNode*>(calleeLiteral->literal())) {
            ...
            if (genericFn && genericFn->header()->isGeneric()) {
                ...
                if (!structDecl && _riu && _riu->sdkFile()) {
                    auto sdkDecl = _riu->sdkFile()->getStructDecl(fnName);
                    if (sdkDecl) {
                        structDecl = sdkDecl;
                        structOwner = _riu->sdkFile();
                    }
                }
                // 处理泛型结构体构造函数的类型推断
                if (structDecl && structDecl->isGeneric() && !node->getTypeArgs().empty()) {
                    ...
                    if (explicitTypeArgs.size() == structDecl->typeParams().size()) {
                        ...
                        if (!structImpl && _riu && _riu->sdkFile() && _riu->sdkFile() != structOwner) {
                            structImpl = _riu->sdkFile()->getStructImpl(fnName);
                        }
                    ...
                    if (_riu && _riu->sdkFile() && _riu->sdkFile() != _file) {
                        sema::resolveCtorOverload(_riu->sdkFile(), fnName, node->getArgs(), node->getLineNumber());
                    }
                } else {
                    // 解析函数重载
                    sema::resolveFnOverload(_file, _riu ? _riu->sdkFile() : nullptr, fnName, node->getArgs(),
                                            node->getLineNumber());
                }
            }
        }
    ...
    if (auto calleeLiteral = dynamic_cast<ExprLiteralNode*>(calleeExpr)) {
        if (auto objLiteral = dynamic_cast<LiteralObjNode*>(calleeLiteral->literal())) {
            ...
            FileNode* structOwner = _file;
            if (!structDecl && _riu && _riu->sdkFile()) {
                auto sdkDecl = _riu->sdkFile()->getStructDecl(ctorFnName);
                if (sdkDecl) {
                    structDecl = sdkDecl;
                    structOwner = _riu->sdkFile();
                }
            ...
            if (structDecl && structDecl->isGeneric() && !node->getTypeArgs().empty()) {
                ...
                if (explicitTypeArgs.size() == structDecl->typeParams().size()) {
                    ...
                    if (!ctorStructImpl && _riu && _riu->sdkFile() && _riu->sdkFile() != structOwner) {
                        ctorStructImpl = _riu->sdkFile()->getStructImpl(ctorFnName);
                    }
                }
            }
        }
    ...
    if (auto calleeLit = dynamic_cast<ExprLiteralNode*>(calleeExpr)) {
        if (auto objLit = dynamic_cast<LiteralObjNode*>(calleeLit->literal())) {
            ...
            if (!delayGenericLambdaArgs) {
                ...
                if (_riu && _riu->sdkFile() && _riu->sdkFile() != _file) {
                    _riu->sdkFile()->collectFnOverloads(fnName, candidates);
                }
                // 按实参 arity 过滤
    ...
    if (auto dotNode = dynamic_cast<ExprDotNode*>(calleeExpr)) {
        ...
        if (!effectiveTypeName.empty() && !isBuiltinType(effectiveTypeName) && !baseType.isArrayGeneric() &&
            ...
            sema::resolveMethodOverload(_file, _riu ? _riu->sdkFile() : nullptr, effectiveTypeName, member,
