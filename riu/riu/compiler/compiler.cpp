...
Compiler::Compiler(llvm::LLVMContext& context, llvm::IRBuilder<>& builder, llvm::Module* mod, FileNode* file, Riu* riu,
                   bool isSdk, bool isTestDll)
    : _context(context), _builder(builder), _module(mod), _file(file), _riu(riu), _isSdk(isSdk), _isTestDll(isTestDll) {
    // 初始化基本类型映射表
...
void Compiler::compile(FileNode* file) {
    ...
    // SDK 需要生成运行时辅助函数（Weak / Heap / Array free；Rc alloc/retain/release 在 riurt）
    if (_isSdk) {
        DEBUG_LOG("Emitting runtime helpers (riu module)");
    ...
    // main 函数会被重命名为 riu_main，真正的 main 由 mainStartup 提供
    if (!_isSdk) {
        if (_isTestDll) {
            DEBUG_LOG("Emitting test DLL init");
            vector<string> loadOrder;
            if (_riu) loadOrder = _riu->loadOrder();
...
//
// 生成内容：
//   @riu_test_count:   i32           — 测试数量
//   @riu_test_names:   [N x ptr]     — 测试名称字符串指针数组
//   @riu_test_fns:     [N x ptr]     — 测试函数指针数组
//   riu_test_get_count(): i32         — dllexport，返回 @riu_test_count
//   riu_test_get_name(i32): ptr       — dllexport，返回 names[i]
//   riu_test_get_fn(i32): ptr         — dllexport，返回 fns[i]
//
// riu-test-runner.exe 加载 DLL 后通过 GetProcAddress 调用访问器获取测试列表，
...
void Compiler::emitTestRegistrations() {
    ...
    if (testFns.empty()) {
        // 没有 #Test 函数：不生成注册表（riu-test-runner.exe 会因找不到 riu_test_get_count 而跳过该 DLL）
        return;
    }
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        auto nameGlobal =
            new llvm::GlobalVariable(*_module, nameConst->getType(), true, llvm::GlobalValue::InternalLinkage,
                                     nameConst, "_riu_test_name_" + std::to_string(i));
    ...
    auto countGlobal =
        new llvm::GlobalVariable(*_module, i32Ty, false, llvm::GlobalValue::InternalLinkage,
                                 llvm::ConstantInt::get(i32Ty, static_cast<uint32_t>(N)), "riu_test_count");
    ...
    new llvm::GlobalVariable(*_module, fnsArrTy, false, llvm::GlobalValue::InternalLinkage, fnsConst, "riu_test_fns");

    // ====== dllexport 访问器函数 ======

    // riu_test_get_count(): i32
    {
        auto fnType = llvm::FunctionType::get(i32Ty, {}, false);
        auto fn = llvm::Function::Create(fnType, llvm::Function::ExternalLinkage, "riu_test_get_count", _module);
    ...
    // riu_test_get_name(i32): ptr
    {
        auto fnType = llvm::FunctionType::get(ptrTy, {i32Ty}, false);
        auto fn = llvm::Function::Create(fnType, llvm::Function::ExternalLinkage, "riu_test_get_name", _module);
        fn->setDLLStorageClass(llvm::GlobalValue::DLLExportStorageClass);
        ...
        auto nameGlobal = _module->getGlobalVariable("riu_test_names", true);
    ...
    // riu_test_get_fn(i32): ptr (返回 void(*)() 函数指针)
    {
        auto fnType = llvm::FunctionType::get(voidFnPtrTy, {i32Ty}, false);
        auto fn = llvm::Function::Create(fnType, llvm::Function::ExternalLinkage, "riu_test_get_fn", _module);
        fn->setDLLStorageClass(llvm::GlobalValue::DLLExportStorageClass);

        auto entry = llvm::BasicBlock::Create(_context, "entry", fn);
        _builder.SetInsertPoint(entry);
        llvm::Value* idx = &*fn->arg_begin();
        auto fnsGlobal = _module->getGlobalVariable("riu_test_fns", true);
...
void Compiler::compileStructDecls() {
    ...
    if (_riu && _riu->sdkFile() && _file != _riu->sdkFile()) {
        for (auto* sdkImp : _riu->sdkFile()->wildcardImports()) {
    ...
    for (auto* imp : _file->wildcardImports()) {
        if (imp == _riu->sdkFile()) continue; // 避免重复处理 SDK
                ...
                if (!fn) {
                    ...
                    for (auto& t : paramTypes) {
                        ...
                        if (t.isPtr() || t.isRef()) {
                            ...
                            if (!sd && _riu && _riu->sdkFile()) {
                                sd = _riu->sdkFile()->getStructDecl(t.name);
                            }
...
// Reflect Type 节点 lazy emit, 由 `__riu_reflect_type:<T>()` intrinsic 调用站调用.
// 符号: __riu_reflect_<sanitized-mod>_<typename>__type, linkonce_odr rodata.
//
// Type layout (由 base.ut struct Type 定义驱动): { String name }
// Field layout (由 base.ut struct Field 定义驱动): { String name }
...
llvm::GlobalVariable* Compiler::ensureReflectTypeGlobal(const TypeInfo& t, llvm::GlobalVariable** outFieldsRefs) {
    ...
    if (_file && _file->getStructDecl(t.name)) {
        ownerFile = _file;
    } else if (_riu && _riu->sdkFile() && _riu->sdkFile() != _file && _riu->sdkFile()->getStructDecl(t.name)) {
        ownerFile = _riu->sdkFile();
    } else if (_file) {
        for (auto* imp : _file->wildcardImports()) {
    ...
    // 符号名: __riu_reflect.<mod>.<type>（LLVM quoted identifier 支持 '.'）
    ...
    std::string symName = "__riu_reflect." + sanitize(ownerFile->moduleName()) + "." + t.name;
    ...
    // ==== Fields refs 数组（GEP 指针 —— 不可简化为 riu 类型表达，保留手搓 LLVM）====
