...
//
// 为 sdk/core/src/riu/core/assert.ut 中以 #Builtin 占位的 4 个断言函数
// 在调用点合成 IR：
...
//
// 失败路径统一为：调 SDK `_riu_test_assert_failed()` → RaiseException(0xE0FA17ED)
// → 由 riu test 的 SEH wrapper 翻译为 ASSERT_FAILED 显示。
...
// 获取或声明断言失败函数
// 始终调用 SDK 的 _riu_test_assert_failed() → RaiseException(0xE0FA17ED)
// 由 riu test (JIT) 或 riu-test-runner.exe (DLL) 的 SEH wrapper 捕获
llvm::Function* getAssertFailedFn(llvm::Module* module, FileNode* file) {
    string modName = "riu.core"; // 兜底
    if (file) {
        auto* fnSym = file->lookupFnSymbol("__riu_test_assert_failed");
        if (fnSym && !fnSym->moduleName.empty()) {
            modName = fnSym->moduleName;
        }
    }
    string name = Mangler::function(modName, "__riu_test_assert_failed", {}, true);
