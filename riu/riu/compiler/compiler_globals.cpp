...
//   并生成 _riu_global_init_<Mod>() 函数在运行期执行初始化。
//
// Phase 3: const-eval 优先分流 —— 若初始化器 const-evaluable，
// 直接 emit LLVM ConstantInitializer（零运行期开销），不进 _riu_global_init。
//
...
void Compiler::compileGlobalVars() {
    ...
    // Phase 6: 始终生成 _riu_global_init_<Mod>() 桩 —— 即使本模块无全局/静态字段，
    ...
    // Phase 6: 始终生成 __riu_global_init.<Mod>() —— 即使无 runtime init item，
    // 跨模块 main shim 也会按拓扑序调用它；空函数开销为一条 ret。
    auto fnName = "__riu_global_init." + _file->moduleName();
