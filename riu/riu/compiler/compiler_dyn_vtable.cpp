...
//
// 符号：`__riu_vtable.<U全限定>.<D全限定>`，linkonce_odr，允许多 TU 共享去重。
//
...
// 把类型/限定名转成符号安全形式：riu 语法字符直通（LLVM quoted identifier 合法）；
...
// 找 U 在 Riu 全局视野下的所属模块名（与 getDestructorFunction 同一规则的简化版本）。
// 普通 struct → 声明该 struct 的文件模块；SDK 内置 → sdkFile 模块；未登记 → 空串。
std::string findStructOwnerModule(Riu* riu, const std::string& structName) {
    if (!riu) return {};
...
// 内置类型（i32 / bool / ...）的 ToString 等 impl 写在 SDK base.ut 里，
// 必须用 impl 所在文件的模块名（如 `riu.core`）才能拿到正确的链接符号。
struct ImplLookup {
    FnNode* method;
    std::string ownerModule;
};

ImplLookup findImplMethod(Riu* riu, const std::string& structName, const std::string& methodName) {
    if (!riu) return {};
...
llvm::GlobalVariable* Compiler::getOrEmitDynVTable(const TypeInfo& concreteType, const std::string& specQualified,
    ...
    for (size_t sigIdx = 0; sigIdx < sigs.size(); ++sigIdx) {
        ...
        auto impl = findImplMethod(_riu, uStruct, methodName);
...
// 的方法 (例如 riu.core#i32_to_string) 实际签名是 (<U> by-value, ...) -> R, 见
...
//   __riu_dyn_thunk__<U>__<specQualified>__<method>(ptr recv, P1, ..., Pn) -> R {
//     v = load <U>, ptr recv
...
llvm::Function* Compiler::getOrEmitDynPrimitiveThunk(const TypeInfo& concreteType, const std::string& specQualified,
    ...
    const std::string methodName = sig->name().getText();
    std::string thunkName = "__riu_dyn_thunk.";
