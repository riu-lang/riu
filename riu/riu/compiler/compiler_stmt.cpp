...
void Compiler::compileDeclareAssignStatement(StatementDeclareAssignNode* node) {
    ...
    if (auto arrayInitNode = dynamic_cast<ExprArrayInitNode*>(expr)) {
        ...
        // Phase 2.5：仅接受 Heap:<T>(...) 等同型 Heap<T> RHS，作用域尾走 __riu_heap_free
        // 注：Heap-from-Heap 拷贝会双释放 — 借用检查 Phase 2.8 静态拒绝
            ...
            auto isKnownTypeName = [&](const string& n) -> bool {
                ...
                if (_riu && _riu->sdkFile() && _riu->sdkFile()->getStructDecl(n)) return true;
                return false;
            };
...
void Compiler::compileStaticFieldSetStatement(StatementStaticFieldSetNode* node) {
    auto r = sema::resolveExprTypeLhs(_file, _riu, node->typePath(), node->getLineNumber(), node->getColumn());
