...
llvm::Value* Compiler::compileCustomTypeUnaryOp(ExprNode* expr, const TypeInfo& type, const string& methodName,
    ...
    if (!methodSymbol && _riu && _riu->sdkFile()) {
        methodSymbol = _riu->sdkFile()->lookupFnSymbolWithParams(methodFullName, methodParamTypes);
    }
...
llvm::Value* Compiler::compileGetRefExpr(ExprGetRefNode* node) {
    ...
    for (auto& sub : subs) {
        ...
        currentPtr = structFieldPtr(structType, currentPtr, static_cast<unsigned>(fieldIndex), "struct.field.ptr");
