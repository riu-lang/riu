    ...
    // sema 已先做形态校验 (#Static 命中 / 缺失 / 实例方法误用), 这里直接 emit call.
    {
        ...
        if (structImpl) {
            ...
            if (!lhsTArgs.empty()) {
                ...
                if (!baseDecl && _riu && _riu->sdkFile() && _riu->sdkFile() != _file) {
                    baseDecl = _riu->sdkFile()->getStructDecl(lhsRaw);
                    baseOwner = _riu->sdkFile();
                }
...
//
// 当前 emit 策略：
//   - vtable 槽：constant null（占位；Phase 3 替换为 `&__riu_vtable_<U>_<D>`）
//   - data 槽：
...
llvm::Value* Compiler::compileDynCtorExpr(ExprDynCtorNode* node) {
    ...
    if (_riu && file && !specBareName.empty()) {
