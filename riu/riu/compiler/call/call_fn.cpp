...
llvm::Value* Compiler::compileFunctionCall(ExprCallNode* callNode, const string& fnName, vector<llvm::Value*>& args,
    ...
    // 命中 SDK assert.ut 的 riu 重载，而不会跑到 #Builtin 的 compileTestAssertEq。
...
llvm::Value* Compiler::compileGenericFunctionCall(ExprCallNode* callNode, const string& fnName,
    ...
    if (genericFn->header()->hasAnno("Builtin")) {
        ...
        sema::validateBuiltinIntrinsicTypeShape(fnName, typeArgs, argTypes, callNode->getArgs(), _file,
                                                _riu ? _riu->sdkFile() : nullptr, callNode->getLineNumber(),
                                                callNode->getColumn());
        ...
        if (fnName == "__riu_reflect_type") {
            ...
            if (!gv) {
                // Phase 3a 仅放行 Normal 用户 / SDK struct；SemaPass 已抛 E6019。
                throwSemaGap(callNode->getLineNumber(), callNode->getColumn());
            }
        ...
        if (fnName == "heap_some" || fnName == "heap_null") {
            ...
            // heap_some<T>(v T) Heap<T>?  → {_has=true,  _value=__riu_heap_alloc + store v}
    ...
    if (!instRetType.empty()) {
        ...
        if (!sd && _riu && _riu->sdkFile()) {
            sd = _riu->sdkFile()->getStructDecl(instRetType.name);
        }
