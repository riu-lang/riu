...
// 模块 .ud：二进制接口 + skeleton 源文本再 parse。
// 格式：magic "RIUD" | u32 version | u64 source_hash | 正文。
...
FileNode* tryLoad(Riu& riu, const std::string& declPath, const std::string& srcAbs, const std::string& moduleName) {
    ...
    try {
        ...
        if (auto sdk = riu.sdkFile(); sdk && sdk != file) {
        ...
        if (!parseSkeletonInto(riu, file, skeleton, srcAbs)) {
            DEBUG_LOG_VAL("  .ud skeleton parse fail", srcAbs);
            delete file;
            return nullptr;
        }
        ...
        DEBUG_LOG_VAL("  .ud load fail", srcAbs << " : " << e.what());
        delete file;
        return nullptr;
    } catch (...) {
        DEBUG_LOG_VAL("  .ud load fail", srcAbs);
        delete file;
        return nullptr;
    }
}
