...
// `riu.io.xxx` 需要当前文件上的 packageChild("riu","io")；不覆盖已有末段别名。
...
std::any ASTBuilder::visitExternDelc(riu::riuParser::ExternDelcContext* ctx) {
    ...
    for (auto header : fnHeaders) {
        ...
        auto throwExternKind = [&](const TypeInfo& t, const char* where) {
            ...
            if (t.isFn()) {
                throw RiuError(declLine, ErrorCode::E2031, fnName, where);
            }
            if (t.isHeap()) {
...
std::any ASTBuilder::visitLetGlobal(riu::riuParser::LetGlobalContext* ctx) {
...
void ASTBuilder::expandPackageWildcard(FileNode* file, const string& pkgModName, int line) {
    ...
    // 没有 pkg：导出所有 .ut 和子目录（默认别名，不扁平）
    // 通配注入的别名在 _wildcardAliasSources 中记录来源；已注入过的同名别名不
    // 立即报错，而是追加来源，留到使用点检查歧义（Phase 5）。
    for (auto& child : _riu.listPackageRiuChildren(pkgModName)) {
...
std::any ASTBuilder::visitImports(riu::riuParser::ImportsContext* ctx) {
    auto file = any_cast_p<FileNode>(stack.back());
    ...
    // `riu.core` 已通过 parent scope 默认注入，跳过。
    if (modName == "riu.core" || modName == file->moduleName()) {
        return nullptr;
    }
    ...
    if (!wildcard) {
        ...
        if (path.segs.size() >= 2 && pathKind == Riu::ModulePathKind::NotFound) {
            ...
            if (parentKind == Riu::ModulePathKind::NotFound && parent) {
                parentKind = Riu::ModulePathKind::File;
            }
            if (!parent && parentKind == Riu::ModulePathKind::File) {
                parent = _riu.loadModule(parentMod, line);
            }
...
std::any ASTBuilder::visitEnumDecl(riu::riuParser::EnumDeclContext* ctx) {
    ...
    for (auto* vCtx : ctx->variants) {
        ...
        if (!enumDecl->addVariant(variant)) {
            _scopeStack.pop_back();
            stack.pop_back();
            throw RiuError(static_cast<int>(vCtx->name->getLine()),
...
std::any ASTBuilder::visitEnumVariant(riu::riuParser::EnumVariantContext* ctx) {
