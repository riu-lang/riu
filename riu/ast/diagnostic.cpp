...
string diagnosticFile(const string& sourcePath, const RiuError& err) {
...
void DiagnosticEngine::emit(std::ostream& out, const string& sourcePath, const RiuError& err) {
    DiagSeverity defaultSev = err.getSeverity();
    // emit 仅服务 warning / note；默认 Error 的码应当 throw 走 renderRiuError 路径
    assert(defaultSev != DiagSeverity::Error &&
           "DiagnosticEngine::emit() called with default-Error code; use throw + renderRiuError instead");
    ...
    // 升级到 Error 时：不在 emit 内渲染, 直接 throw 让顶层 catch 走 renderRiuError
...
void DiagnosticEngine::emit(const string& sourcePath, const RiuError& err) {
    emit(std::cerr, sourcePath, err);
}

void DiagnosticEngine::renderRiuError(std::ostream& out, const string& sourcePath, const RiuError& err) {
    Diagnostic d;
