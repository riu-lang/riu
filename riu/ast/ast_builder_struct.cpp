...
std::any ASTBuilder::visitStructDecl(riu::riuParser::StructDeclContext* ctx) {
    ...
    if (!isSpec) {
        for (auto* a : ctx->buildAnnos) {
            if (a->name->getText() == "DraftLike") {
                throw RiuError(static_cast<int>(a->name->getLine()),
    ...
    if (isSpec) {
        if (ctx->fnClean()) {
            auto* fc = ctx->fnClean();
            throw RiuError(static_cast<int>(fc->getStart()->getLine()),
        ...
        for (auto* fnCtx : ctx->fn()) {
            ...
            if (header->isGeneric()) {
                ...
                if (isDraftLike) {
                    throw RiuError(hLine, hCol, ErrorCode::E1112, structName);
                }
                throw RiuError(hLine, hCol, ErrorCode::E1104, structName, header->name().getText());
            }

            // DRAFT-spec-default-body Phase 1：spec body 内方法可带可选默认体；
            // 体内 `$` 绑 Self 抽象类型变量（Phase 2 sema 完成占位符号校验）。
            // 注: T* = T* 裸指针, 必须显式 nullptr 初始化, 否则上传垃圾。
        ...
        if (isStatic) {
            ...
            for (auto* a : fieldCtx->buildAnnos) {
                ...
                if (annoName != "Static" && annoName != "Mut" && annoName != "Cval" && annoName != "Inline") {
                    ...
                    throw RiuError(static_cast<int>(tk->getLine()), static_cast<int>(tk->getCharPositionInLine()) + 1,
            ...
            // #Inline 必须与 #Cval 组合
            if (isInline && !isCval) {
                throw RiuError(static_cast<int>(fieldCtx->name->getLine()),
    ...
    for (auto fnCtx : ctx->fn()) {
        ...
        if (!fnCtx->fnBody()) {
            if (!header->hasAnno("Builtin")) {
                throw RiuError(header->getLineNumber(), header->getColumn(), ErrorCode::E2007, structName,
                               header->name().getText())
...
std::any ASTBuilder::visitFnClean(riu::riuParser::FnCleanContext* ctx) {
...
FieldAnnoFlags readFieldAnnos(const std::vector<riu::riuParser::BuildAnnoContext*>& annos) {
    FieldAnnoFlags r;
    for (auto* a : annos) {
        ...
        if (a->annoArg() != nullptr) {
            ...
            throw RiuError(line, col, ErrorCode::E3108, name);
        }
        if (name == "Val") {
            if (r.isFrozen) throw RiuError(line, col, ErrorCode::E3108, name);
            r.isVal = true;
        } else if (name == "Frozen") {
            if (r.isVal) throw RiuError(line, col, ErrorCode::E3108, name);
            r.isFrozen = true;
        } else if (name == "Static") {
    ...
    if (r.isInline && !r.isCval) {
        throw RiuError(0, 0, ErrorCode::E3117);
    }
    return r;
}
...
std::any ASTBuilder::visitFiledDecl(riu::riuParser::FiledDeclContext* ctx) {
    auto parent = currentScope();
