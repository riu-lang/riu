...
#ifndef RIU_LANG_MANGLER_H
#define RIU_LANG_MANGLER_H

#include "types.h"

// Mangler 集中管理所有用户/SDK 源符号的 LLVM 名称生成。
// 命名规则（仅作用于 .ut 源文件中定义的符号；运行时辅助、Windows API、
...
// Fallible 签名：`fallibleErrType` 非空时在形参列表末追加返回槽（`T!E` 或 void 成功 `!E`），
// 与 DRAFT-fallible-sig.md §6 一致；ABI 仍由 wrapFallibleRetType 决定，不参与 mangle。
// 模块路径中的 "." 与符号名中的 "." 自然融合（如 riu.core.base.foo）。
class Mangler {
public:
    // 形参 + fallible 返回槽（仅 fallibleErrType 非空时追加末位）
    static vector<TypeInfo> fnSignatureTypes(const vector<TypeInfo>& params, const TypeInfo& retType,
    ...
    static string function(const string& module, const string& name, const vector<TypeInfo>& params, bool isPrivate,
                           const TypeInfo& retType = TypeInfo(), const string& fallibleErrType = "");
    ...
    static string method(const string& module, const string& structName, const string& methodName,
                         const vector<TypeInfo>& params, bool isPrivate, const TypeInfo& retType = TypeInfo(),
                         const string& fallibleErrType = "");
