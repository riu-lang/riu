...
[[noreturn]] void throwAmbiguousBareType(const string& name, const vector<TypePathResult>& cands, int line, int col) {
    ...
    throw RiuError(static_cast<size_t>(ln), col, ErrorCode::E5015, name, sources);
}
...
TypePathResult resolveTypePath(FileNode* file, Riu* riu, const TypePath& path, int line, int col) {
    ...
    if (path.isBare()) {
        ...
        FileNode* sdk = riu ? riu->sdkFile() : nullptr;
        if (sdk) {
            vector<TypePathResult> cands;
            std::set<string> seen;
            for (auto* imp : sdk->wildcardImports()) {
