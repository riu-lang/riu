...
TypeInfo ExprCallNode::structuralType() const {
    ...
    if (auto dot = dynamic_cast<ExprDotNode*>(_calleeExpr)) {
        ...
        if (dot->member() == "inv" && recv.isFloat()) {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3070, recv.name);
        }
    ...
    if (scope) {
        ...
        if (sym && sym->kind != SymbolKind::Function && sym->kind != SymbolKind::TypeParam) {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3095, sym->name);
        }
...
TypeInfo ExprAddSubNode::structuralType() const {
    ...
    if (leftType != rightType) {
        ...
        throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3001, "arithmetic", leftType.name,
                       rightType.name);
    }
    return leftType;
}
...
TypeInfo ExprMulDivModNode::structuralType() const {
    ...
    if (leftType != rightType) {
        ...
        throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3001, "mul/div/mod", leftType.name,
                       rightType.name);
    }
    return leftType;
}
...
TypeInfo ExprBinOpNode::structuralType() const {
    ...
    if (leftType != rightType) {
        ...
        throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3001, "bitwise", leftType.name,
                       rightType.name);
    }
    return leftType;
}
...
TypeInfo ExprDotNode::structuralType() const {
    ...
    if (_safe) {
        ...
        if (!baseT.isNullable()) {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3024, baseT.name);
        }
        auto innerType = baseT.nullableInnerType();
        if (!innerType) {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3050);
        }
        ...
        // 字段和方法都未找到 → 报错
        if (sd) {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3040, innerType->name, member);
        } else {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3044, innerType->name);
        }
    }
    ...
    if (member == "value") {
        ...
        if (baseT.name == "Field") {
            ...
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3133);
        }
    ...
    if (!member.empty() && std::ranges::all_of(member, [](char c) { return c >= '0' && c <= '9'; })) {
        ...
        if (resolved.isTuple()) {
            ...
            if (idx >= elems.size()) {
                throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3100, member, baseType.getFullName(),
...
TypeInfo ExprCompareNode::structuralType() const {
    ...
    if (leftType != rightType) {
        ...
        throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3001, "comparison", leftType.name,
                       rightType.name);
    }
    return TypeInfo("bool");
}
...
TypeInfo ExprIfElseNode::structuralType() const {
    ...
    auto consider = [&](StatementBlockNode* block) -> bool {
        ...
        if (!blockValueTypesMatch(t, resultType)) {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3005, resultType.name, t.name);
        }
...
TypeInfo ExprOneLineIfElseNode::structuralType() const {
    ...
    if (!blockValueTypesMatch(trueType, falseType)) {
        throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3005, trueType.name, falseType.name);
    }
...
TypeInfo ExprGetNode::structuralType() const {
    ...
    if (!arrayType.isArray()) {
        throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3062, arrayType.name);
    }
...
TypeInfo ExprArrayNode::structuralType() const {
    ...
    for (size_t i = 1; i < _elements.size(); ++i) {
        ...
        if (elemType != elementType) {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3009, elementType.name, elemType.name);
        }
    }
...
TypeInfo ExprArrayInitNode::structuralType() const {
    ...
    if (_explicitType) {
        ...
        if (valueType != elementType) {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3009, valueType.name, elementType.name);
        }
    } else {
...
TypeInfo ExprGetRefNode::structuralType() const {
    auto scope = findNearestScope();
    if (!scope) {
        throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3097);
    }
    ...
    for (auto& sub : _subs) {
        ...
        if (!file) {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3043);
        }
        ...
        if (!structDecl) {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3041, lookupType.name);
        }

        auto field = structDecl->field(sub.getText());
        if (!field) {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3040, lookupType.name, sub.getText());
        }
...
TypeInfo ExprUnaryNode::structuralType() const {
    ...
    if (isBuiltinType(rightType.name)) {
        if (_op == Op::Rev && rightType.isFloat()) {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3070, rightType.name);
        }
        if (_op == Op::Not && rightType.name != "bool") {
            throw RiuError(resolveLineNumber(), resolveColumn(), ErrorCode::E3071, rightType.name);
        }
    }
    return rightType;
}
