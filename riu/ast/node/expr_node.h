...
#ifndef RIU_LANG_EXPR_NODE_H
#define RIU_LANG_EXPR_NODE_H
...
public:
    ...
    void addArg(ExprNode* arg) { _args.push_back(arg); }

    void setTypeArgs(vector<TypeNode*> args) { _typeArgs = std::move(args); }
    [[nodiscard]] const vector<TypeNode*>& getTypeArgs() const { return _typeArgs; }
...
public:
    explicit ExprLiteralNode(Node* parent, LiteralNode* literal) : ExprNode(parent), _literal(literal) {
...
public:
    ExprAddSubNode(Node* parent, Op op, ExprNode* left, ExprNode* right)
...
public:
    ExprMulDivModNode(Node* parent, Op op, ExprNode* left, ExprNode* right)
        : ExprNode(parent), _op(op), _left(left), _right(right) {}
...
public:
    ExprBinOpNode(Node* parent, Op op, ExprNode* left, ExprNode* right)
        : ExprNode(parent), _op(op), _left(left), _right(right) {}
...
public:
    ExprCompareNode(Node* parent, Op op, ExprNode* left, ExprNode* right)
        : ExprNode(parent), _op(op), _left(left), _right(right) {}
...
public:
    ExprElIfNode(Node* parent, ExprNode* condition, StatementBlockNode* block)
        : Node(parent), _condition(condition), _block(block) {}

    [[nodiscard]] ExprNode* condition() const;
    [[nodiscard]] StatementBlockNode* block() const;
};

class ExprIfElseNode : public ExprNode {
    ExprNode* _condition;
    StatementBlockNode* _thenBlock;
    vector<ExprElIfNode*> _elifs;
    StatementBlockNode* _elseBlock;

public:
    ExprIfElseNode(Node* parent, ExprNode* condition, StatementBlockNode* thenBlock, vector<ExprElIfNode*> elifs,
...
class ExprOneLineIfElseNode : public ExprNode {
    ExprNode* _condition;
    ExprNode* _trueValue;
    ExprNode* _falseValue;

public:
    ExprOneLineIfElseNode(Node* parent, ExprNode* condition, ExprNode* trueValue, ExprNode* falseValue)
...
public:
    ExprGetNode(Node* parent, ExprNode* arrayExpr, vector<ExprNode*> indices)
...
class ExprArrayInitNode : public ExprNode {
    LiteralNode* _value;
    TypeNode* _explicitType;

public:
    ExprArrayInitNode(Node* parent, LiteralNode* value, TypeNode* explicitType)
...
public:
    ExprUnaryNode(Node* parent, Op op, ExprNode* right) : ExprNode(parent), _op(op), _right(right) {}
...
private:
    ...
    TypeNode* _retType; // 仅 Paren 显式标注；其余 nullptr
    TypeNode* _fallibleErrType = nullptr;
    ExprNode* _bodyExpr;               // Form::Expr
    vector<StatementNode*> _bodyStmts; // Form::Block
...
public:
    LambdaExprNode(Node* parent, Form form, vector<LambdaParamSlot> params, TypeNode* retType, ExprNode* bodyExpr,
                   vector<StatementNode*> bodyStmts)
        : ExprNode(parent), _form(form), _params(std::move(params)), _retType(retType), _bodyExpr(bodyExpr),
          _bodyStmts(std::move(bodyStmts)) {}

    void setBodyScope(ScopeNode* sc) { _bodyScope = sc; }
    [[nodiscard]] ScopeNode* bodyScope() const { return _bodyScope; }
...
public:
    ...
    [[nodiscard]] const vector<ExprNode*>& args() const { return _args; }
    [[nodiscard]] const vector<TypeNode*>& lhsTypeArgs() const { return _lhsTypeArgs; }
    [[nodiscard]] const vector<TypeNode*>& rhsTypeArgs() const { return _rhsTypeArgs; }
    [[nodiscard]] bool errPropagate() const { return _errPropagate; }
...
public:
    // 兜底分支
    EnumPatternNode(Node* parent, const Token& elseTok)
...
class MatchArmNode : public ScopeNode {
    EnumPatternNode* _pattern;
    ExprNode* _body;            // 表达式体；块体时为 nullptr
    StatementBlockNode* _block; // 块体；表达式体时为 nullptr

public:
    MatchArmNode(Node* parent, EnumPatternNode* pattern, ExprNode* body, StatementBlockNode* block = nullptr)
        : ScopeNode(parent), _pattern(pattern), _body(body), _block(block) {}

    [[nodiscard]] EnumPatternNode* pattern() const { return _pattern; }
...
class ExprMatchNode : public ExprNode {
    ExprNode* _scrutinee;
    vector<MatchArmNode*> _arms;

public:
    ExprMatchNode(Node* parent, ExprNode* scrutinee, vector<MatchArmNode*> arms)
...
class ExprTryCatchNode : public ExprNode {
    StatementBlockNode* _tryBlock;
    vector<CatchArmNode*> _catches;

public:
    ExprTryCatchNode(Node* parent, StatementBlockNode* tryBlock, vector<CatchArmNode*> catches)
...
class ExprDynCtorNode : public ExprNode {
    TypeNode* _specType; // turbofish 内的类型节点（D 或 D&）
    ExprNode* _arg;      // 构造源：Rc<U> 或 U&
    bool _isBorrow;      // true = Dyn<D&>(...), false = Dyn<D>(...)

public:
    ExprDynCtorNode(Node* parent, TypeNode* specType, ExprNode* arg, bool isBorrow)
...
public:
    ExprMoveAssignNode(Node* parent, ExprNode* left, ExprNode* right) : ExprNode(parent), _left(left), _right(right) {}
...
public:
    ExprNullElseNode(Node* parent, ExprNode* left, ExprNode* right) : ExprNode(parent), _left(left), _right(right) {}
