...
// 诊断引擎（0 LLVM）。parse 与 Sema 共用，放 ast，避免 riu_ast include frontend。
//
// 负责把 RiuError 渲染成统一格式的错误信息：
//   path/to/file.ut:12:5 [E0000] error: 主信息
//      |
//   12 | val x i32 = "hello"
//      |     ^
//
...
struct Diagnostic {
    ...
    int col = 0;           // 1-based UTF-8 字节列；0 = 未知
    string message;
...
public:
    // 把 RiuError 转成 Diagnostic 并渲染到 out。
    // sourcePath 是正在编译的入口文件（回退用）；若 err.file() 非空，用出错节点
    // 所属文件（spec 默认体 fall-through 等跨文件 AST）。
    // 引擎内部按最终路径缓存源文件内容（同一个引擎实例多次调用同一文件不重复读）。
    static void renderRiuError(std::ostream& out, const string& sourcePath, const RiuError& err);
    ...
    //
    // 与 renderRiuError 的区别：renderRiuError 由顶层 catch 调用（错误已经把当前文件
    // 编译截断了），emit 是诊断的**生产者**直接调用 —— 用 RiuError 复用其消息模板
    ...
    // 同一 RiuError，让调用方走原有"首条 error 终止文件"协议。
    //
    // 重入安全：emit 内部对同一进程持有去重 set（path+code+line+col+message），同一
    // 站点只渲染一次。
    //
    // 不应用到 Error 默认严重度的 RiuError —— 那种应当 `throw`，由顶层 catch 走
    // renderRiuError。emit 内部 assert 默认严重度 ≤ Warning。
    static void emit(std::ostream& out, const string& sourcePath, const RiuError& err);

    // 便利重载：默认 std::cerr
    static void emit(const string& sourcePath, const RiuError& err);

    // 测试 / LSP 重启场景清理去重 set
    static void resetEmitDedup();
};

#endif // RIU_LANG_DIAGNOSTIC_H
