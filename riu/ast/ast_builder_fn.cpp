...
std::any ASTBuilder::visitFn(riu::riuParser::FnContext* ctx) {
    ...
    // ==================== #Test 注解校验 (spec §11.3) ====================
    // 仅 *.test.ut 允许；与 #Builtin 互斥；签名 `fn name(): void`、必须有体。
    if (header->hasAnno("Test")) {
        ...
        if (header->hasAnno("Builtin")) {
            throw RiuError(annoLine, annoCol, ErrorCode::E2013, fnName);
        }
        bool sigOk = true;
        // 不允许有参数
        ...
        if (!sigOk) {
            throw RiuError(annoLine, annoCol, ErrorCode::E2012, fnName, fnName);
        }
    }
...
std::any ASTBuilder::visitFnHeader(riu::riuParser::FnHeaderContext* ctx) {
...
std::any ASTBuilder::visitFnParams(riu::riuParser::FnParamsContext* ctx) {
...
std::any ASTBuilder::visitFnParam(riu::riuParser::FnParamContext* ctx) {
...
bool readParamAnnos(const std::vector<riu::riuParser::ParamAnnoContext*>& annos) {
    bool frozen = false;
    for (auto* a : annos) {
...
std::any ASTBuilder::visitFnParamStd(riu::riuParser::FnParamStdContext* ctx) {
...
std::any ASTBuilder::visitFnParamGroup(riu::riuParser::FnParamGroupContext* ctx) {
    Node* parent = any_cast_p<FnHeaderNode>(stack.back());
    auto type = any_cast_p<TypeNode>(visit(ctx->type()));
    bool frozen = readParamAnnos(ctx->paramAnnos);
