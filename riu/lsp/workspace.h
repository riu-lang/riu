...
//
// 设计（P2 Plan A）：
// - 每打开一个文档 → 找最近 riu.toml 作为项目根；找不到则按单文件项目处理
//   （key = 文件绝对路径），保证不互相串扰
...
public:
    ...
    // 项目模式：rootDir 须含 riu.toml；entry 由 toml 决定
    // 单文件模式：rootDir 为文件所在目录，mainAbsPath 即文件
    // 两者构造完后状态一致：_riu 已 init、若成功则 _mainFile 非空
    ...
    [[nodiscard]] FileNode* sdkFile() const { return _riu ? _riu->sdkFile() : nullptr; }
...
private:
    ...
    // 给定文档绝对路径，找到最近 riu.toml 所在目录；找不到返回空串
