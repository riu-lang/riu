...
// LSP 静态补全表实现
// 列表来源：riu-vscode/src/extension.ts 中的 KEYWORDS / TYPES / BUILTIN_FUNCTIONS
...
const std::vector<CompletionItem>& buildItems() {
    static const std::vector<CompletionItem> items = [] {
        ...
        auto kw = [&](std::string label, std::string detail) {
            v.push_back({.label = std::move(label),
        ...
        // SNIPPETS
        sn("fn", "函数声明模板", "fn ${1:name}(${2:params}) ${3:retType} {\n\t${4:body}\n}", "创建一个新函数");
        sn("fn-expr", "函数表达式模板", "fn ${1:name}(${2:params}) = ${3:expr}", "创建一个表达式函数");
        sn("if", "if 条件模板", "if ${1:condition} {\n\t${2:body}\n}", "创建一个 if 条件语句");
        sn("if-else", "if-else 条件模板", "if ${1:condition} {\n\t${2:body}\n} else {\n\t${3:elseBody}\n}",
           "创建一个 if-else 条件语句");
        sn("if-elif-else", "if-elif-else 条件模板",
           "if ${1:condition} {\n\t${2:body}\n} elif ${3:condition2} {\n\t${4:body2}\n} else {\n\t${5:elseBody}\n}",
           "创建一个完整的 if-elif-else 条件语句");
        sn("struct", "结构体声明模板", "struct ${1:Name} {\n\t${2:field} ${3:type}\n}", "创建一个结构体声明");
        sn("impl", "结构体实现模板", "${1:Name} {\n\tfn ${2:method}(${3:params}) {\n\t\t${4:body}\n\t}\n}",
           "创建一个结构体实现块");
        ...
        sn("let", "变量声明模板（默认浅不可变）", "let ${1:name} ${2:type} = ${3:value}",
           "创建一个不可变变量声明（默认 let）");
        sn("let-mut", "可变变量声明模板", "#Mut let ${1:name} ${2:type} = ${3:value}",
           "创建一个可变变量声明（#Mut let）");
        sn("let-cval", "编译时常量声明模板", "#Cval let ${1:name} ${2:type} = ${3:value}",
           "创建一个编译时常量声明（#Cval let）");
        sn("let-global", "全局常量声明模板", "#Cval\nlet ${1:NAME} ${2:type} = ${3:value}",
           "创建一个全局常量（全局 let 必须 #Cval）");
        sn("ret", "返回语句模板", "ret ${1:value}", "创建一个返回语句");
        sn("array", "数组类型模板", "[ ${1:type} * ${2:count} ]", "创建一个数组类型声明");
        sn("array-lit", "数组字面量模板", "[ ${1:element1}, ${2:element2} ]", "创建一个数组字面量");
        sn("loop", "循环模板", "loop {\n\t${1:body}\n\tif ${2:condition} {\n\t\tbreak;\n\t}\n}", "创建一个循环");
        sn("try", "try-catch 模板", "try {\n\t${1:body}\n} catch ${2:e} ${3:Err} {\n\t${4:handler}\n}",
           "创建一个 try-catch 错误处理块");
        sn("match", "match 模板", "match ${1:expr} {\n\t${2:Pattern} => ${3:result}\n\telse => ${4:default}\n}",
           "创建一个 match 模式匹配");
        sn("enum", "enum 声明模板", "enum ${1:Name} {\n\t${2:Variant1}\n\t${3:Variant2}(${4:T})\n}",
           "创建一个 enum 声明");
